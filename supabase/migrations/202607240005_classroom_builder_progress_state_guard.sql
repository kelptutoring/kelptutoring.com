-- Phase 5.F.5 forward fix: retained dropped Curriculum rows belong to staff
-- Schedule history, but they are not active Sessions and therefore cannot be
-- passed to the active Studied aggregation function.
...
comment on function public.get_my_course_schedule_builder_context(uuid) is
  'Returns the assigned Tutor/Mentor Classroom Builder context, including exact active Studied locks and retained historical rows for safe successor publication.';
