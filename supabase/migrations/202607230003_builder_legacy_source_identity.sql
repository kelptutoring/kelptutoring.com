-- Phase 5.E.4 repair: legacy Schedule placeholders predate Track-source
-- metadata. They remain immutable, explicitly dropped history when a Builder
-- successor is published. New Curriculum Sessions still require complete
-- governed Track identity.
...
begin
  ...
  if exists (
    with proposed as (
      select
        item,
        prior_item.id is not null as retained,
        prior_item.curriculum_node_id as prior_curriculum_node_id,
        prior_item.source_snapshot as prior_source_snapshot
      from jsonb_array_elements(p_items) item
      left join public.course_schedule_items prior_item
        on prior_item.version_id = p_expected_version_id
    ...
    where coalesce(item ->> 'kind', 'curriculum_topic') = 'curriculum_topic'
      and (
        ...
        or (
          retained
          and (
            nullif(item ->> 'curriculumNodeId', '')::uuid
              is distinct from prior_curriculum_node_id
            or (
              ...
              and nullif(btrim(coalesce(
                ...
                prior_source_snapshot ->> 'sourceTrackKey',
                prior_source_snapshot ->> 'trackId',
                prior_source_snapshot ->> 'trackKey',
                ''
              )), '')
            )
            or (
              ...
              and nullif(btrim(coalesce(
                ...
                prior_source_snapshot ->> 'sourceModuleKey',
                prior_source_snapshot ->> 'moduleId',
                prior_source_snapshot ->> 'moduleKey',
                ''
              )), '')
            )
            or (
              ...
              and nullif(btrim(coalesce(
                ...
                prior_source_snapshot ->> 'sourceSessionKey',
                prior_source_snapshot ->> 'sourceSessionId',
                prior_source_snapshot ->> 'sessionId',
                ''
              )), '')
            )
            or (
              ...
              and nullif(btrim(coalesce(
                ...
                prior_source_snapshot ->> 'sourceContentVersionKey',
                prior_source_snapshot ->> 'contentVersionKey',
                prior_source_snapshot ->> 'contentVersion',
                ''
              )), '')
            )
            or (
              nullif(btrim(coalesce(item ->> 'sourceSubjectSlug', '')), '') is not null
              and nullif(btrim(item ->> 'sourceSubjectSlug'), '')
                is distinct from nullif(btrim(coalesce(
                  prior_source_snapshot ->> 'sourceSubjectSlug',
                  ''
                )), '')
            )
            or (
              nullif(btrim(coalesce(item ->> 'sourceTrackSlug', '')), '') is not null
              and nullif(btrim(item ->> 'sourceTrackSlug'), '')
                is distinct from nullif(btrim(coalesce(
                  prior_source_snapshot ->> 'sourceTrackSlug',
                  ''
                )), '')
            )
            or (
              item ? 'difficulty'
              and nullif(btrim(coalesce(item ->> 'difficulty', '')), '')
                is distinct from nullif(btrim(coalesce(
                  prior_source_snapshot ->> 'difficulty',
                  ''
                )), '')
            )
            or (
              item ? 'planningHref'
              and nullif(btrim(coalesce(item ->> 'planningHref', '')), '')
                is distinct from nullif(btrim(coalesce(
                  prior_source_snapshot ->> 'planningHref',
                  prior_source_snapshot ->> 'planningRoute',
                  ''
                )), '')
            )
            or (
              item ? 'resources'
              and (item -> 'resources') is distinct from coalesce(
                prior_source_snapshot -> 'resources',
                '[]'::jsonb
              )
            )
          )
...
comment on function public.publish_course_builder_schedule(
  ...
  'Phase 5.E.4 governed Builder adapter. Retained legacy placeholders preserve their original missing Track identity; supplied retained identity is immutable, and new Curriculum Sessions require the Course focus plus complete Track source identity.';
