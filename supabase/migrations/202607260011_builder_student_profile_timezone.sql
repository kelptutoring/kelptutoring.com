-- Phase 5.G follow-up: the Course Schedule Builder uses the Student's
-- governed Profile timezone for the next Version without rewriting history.
...
begin
  ...
  select jsonb_build_object(
    ...
    'course', jsonb_build_object(
      ...
      'studentTimeZone', coalesce(
        nullif(btrim(preferences.time_zone), ''),
        version.time_zone,
        'UTC'
      ),
  ...
  left join public.user_preferences preferences
    on preferences.user_id = course.student_id
...
comment on function public.get_my_course_schedule_builder_context(uuid) is
  'Returns the active immutable Schedule Version plus the current governed Student Profile timezone used to author its successor.';
