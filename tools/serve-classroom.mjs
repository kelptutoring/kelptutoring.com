...
const preferredPort = Number(process.env.PORT || 4181);
...
function start(port) {
  ...
  activeServer.listen(port, "127.0.0.1", () => {
    const url = `http://127.0.0.1:${port}/src/app/classroom/classroom.html`;
    console.log(`Kelp Classroom is running at ${url}`);
    ...
    console.log("Keep this window open while using the local classroom.");
...
async function runSelfTest(url) {
  try {
    ...
    if (!response.ok || !body.includes("Kelp Classroom")) {
      throw new Error(`Unexpected response: ${response.status}`);
    }

    console.log("Self-test passed: classroom page served successfully.");
  } catch (error) {
