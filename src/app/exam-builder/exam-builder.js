...
let previewRenderHasRun = false;
let previewRefreshInProgress = false;
...
const BUILDER_MUTATING_ACTIONS = new Set([
  "move-question-up",
  "move-question-down",
  "duplicate-question",
  "remove-question",
...
function markBuilderDirty() {
  ...
  document.documentElement.dataset.builderUnsaved = "true";
}
...
function handleBuilderMutationClick(event) {
  if (event.target.closest?.("#addQuestionBtn")) {
    markBuilderDirty();
    return;
  }
  const action = event.target.closest?.("[data-action]")?.dataset?.action;
  if (BUILDER_MUTATING_ACTIONS.has(action)) markBuilderDirty();
}
...
const els = {
  ...
  previewPreviousBtn: document.getElementById("previewPreviousBtn"),
  ...
  previewRefreshStatus: document.getElementById("previewRefreshStatus"),
...
function createQuestion(overrides = {}) {
  return {
    ...
    answer: "",
    options: ["", "", "", ""],
    optionGraphs: [null, null, null, null],
    optionImages: ["", "", "", ""],
    ...
    graphAfterText: "",
    graph: null,
    pdfAnswerSpaceSize: "medium",
    pdfAnswerSpaceCustomMm: 80,
    numericExpectedAnswer: "",
    numericExactMatch: false,
    numericTolerance: 1e-6,
    numericAngleMode: "radians",
...
function cloneQuestionValue(value) {
  if (typeof structuredClone === "function") {
    try {
      return structuredClone(value);
    } catch (_) {
      // Fall back to JSON for the plain data stored by the exam builder.
    }
  }
...
function estimateOnlineRenderPayload(exam) {
  const serialized = JSON.stringify(exam || {});
  const contributors = [];
  const addContributor = (label, type, value) => {
    if (!value) return;
    const payload = typeof value === "string" ? value : JSON.stringify(value);
    const bytes = estimateStringBytes(payload);
    if (bytes > 0) contributors.push({ label, type, bytes });
  };

  (exam.questions || []).forEach((question, questionIndex) => {
    const questionLabel = `Question ${questionIndex + 1}`;
    addContributor(`${questionLabel} body image`, "image", question.imageData);
    (question.optionImages || []).forEach((image, optionIndex) => {
      addContributor(`${questionLabel} option ${optionLetter(optionIndex)} image`, "image", image);
    });
    addContributor(`${questionLabel} body diagram`, "diagram", question.graph);
    (question.optionGraphs || []).forEach((graph, optionIndex) => {
      addContributor(`${questionLabel} option ${optionLetter(optionIndex)} diagram`, "diagram", graph);
    });
    addContributor(`${questionLabel} text/settings`, "question data", {
      name: question.name,
      type: question.type,
  ...
  addContributor("Exam title, instructions, and settings", "exam data", {
    title: exam.title,
    madeBy: exam.madeBy,
    subject: exam.subject,
    instructions: exam.instructions,
    durationMinutes: exam.durationMinutes,
    titleColor: exam.titleColor,
    stripeColor: exam.stripeColor
  });

  contributors.sort((a, b) => b.bytes - a.bytes);
  return {
    totalBytes: estimateStringBytes(serialized),
    contributors
  };
}

function buildOnlineRenderSizeWarning(audit) {
  const limit = formatByteSize(ONLINE_RENDER_SIZE_LIMIT_BYTES);
  const topItems = audit.contributors.slice(0, 8);
  const list = topItems.length
    ? topItems.map((item, index) => `${index + 1}. ${item.label} (${item.type}) - ${formatByteSize(item.bytes)}`).join("\n")
    : "No large individual item was detected.";

  return [
    `This exam is about ${formatByteSize(audit.totalBytes)}, which is above the ${limit} browser preview budget.`,
    "The student view may fail to open until the largest media files are compressed or moved to backend storage.",
    "",
    "Largest contributors:",
    list,
    "",
    "Try opening the student view anyway?"
  ].join("\n");
}

function buildOnlineRenderStorageFailureMessage(audit) {
  const topItems = (audit?.contributors || []).slice(0, 8);
  const list = topItems.length
    ? topItems.map((item, index) => `${index + 1}. ${item.label} (${item.type}) - ${formatByteSize(item.bytes)}`).join("\n")
    : "No large individual item was detected.";
  ...
  return [
    "The student view could not be opened because this browser could not store the attached media.",
    audit ? `Estimated exam size: ${formatByteSize(audit.totalBytes)}.` : "",
    "",
    "Largest contributors:",
    list,
    "",
    "Try compressing or removing the largest images. Diagrams are usually much smaller than uploaded images."
  ].filter(Boolean).join("\n");
}
...
function truncateDisplayText(value, maxLength = QUESTION_NAME_MAX_LENGTH) {
  const text = String(value || "").trim();
  return text.length > maxLength ? `${text.slice(0, Math.max(0, maxLength - 3)).trim()}...` : text;
}

function getQuestionDisplayName(question) {
  const name = normalizeQuestionName(question?.name);
  if (name) return truncateDisplayText(name);
  const prompt = String(question?.prompt || "").trim();
  return prompt ? truncateDisplayText(firstWords(prompt, 8)) : "Untitled question";
}
...
const NUMERIC_TOLERANCE_OPTIONS = [
  ["0.5", "Whole number"],
  ["0.05", "Tenths"],
  ["0.005", "Hundredths"],
  ["0.0005", "Thousandths"],
  ["0.0000005", "Millionths"]
];
...
function getQuestionTypeValue(questionOrType) {
  return typeof questionOrType === "string" ? questionOrType : questionOrType?.type;
}

function questionBaseType(questionOrType) {
  const type = getQuestionTypeValue(questionOrType);
...
function getPdfAnswerSpaceOptionsForType(type) {
  ...
  if (type === "essay") {
    return [
      ...
      ["medium", "Medium block"],
      ["large", "Large block"],
      ["custom", "Specific distance"]
    ];
  }
  return [];
}

function normalizePdfAnswerSpaceSize(value, type = "essay") {
  const options = getPdfAnswerSpaceOptionsForType(type);
  if (options.length) {
    const allowed = new Set(options.map(([optionValue]) => optionValue));
    if (allowed.has(value)) return value;
    return type === "short-answer" ? "small" : "medium";
  }
  return ["none", "small", "medium", "large", "custom"].includes(value) ? value : "medium";
}

function normalizePdfAnswerSpaceCustomMm(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return 80;
  return Math.max(10, Math.min(260, parsed));
}
...
function normalizeNumericToleranceChoice(value) {
  const tolerance = normalizeNumericTolerance(value);
  let closest = NUMERIC_TOLERANCE_OPTIONS[NUMERIC_TOLERANCE_OPTIONS.length - 1][0];
  let closestDistance = Infinity;
  NUMERIC_TOLERANCE_OPTIONS.forEach(([optionValue]) => {
    const distance = Math.abs(Math.log10(tolerance) - Math.log10(Number(optionValue)));
...
function normalizeNumericAngleMode(value) {
  return value === "degrees" ? "degrees" : "radians";
}
...
const PDF_ANSWER_SPACE_SIZES_MM = {
  small: 35,
  medium: 60,
  large: 95
};

function getPdfAnswerSpaceHeightMm(question) {
  const size = normalizePdfAnswerSpaceSize(question?.pdfAnswerSpaceSize, question?.type);
  if (size === "custom") return normalizePdfAnswerSpaceCustomMm(question?.pdfAnswerSpaceCustomMm);
  return PDF_ANSWER_SPACE_SIZES_MM[size] || PDF_ANSWER_SPACE_SIZES_MM.medium;
}
...
function syncQuestionOptionAssets(question) {
  const optionCount = Array.isArray(question.options) ? question.options.length : 0;
  question.optionGraphs = normalizeOptionGraphs(question.optionGraphs, optionCount);
  question.optionImages = normalizeOptionImages(question.optionImages, optionCount);
  return question;
}
...
function initialize() {
  ...
  if (returnSession) {
    ...
    if (layout) layout.classList.toggle("preview-collapsed", returnSession.previewCollapsed !== false);
    requestAnimationFrame(() => {
      if (els.examPreview) els.examPreview.scrollTop = 0;
      window.scrollTo({ top: Math.max(0, Number(returnSession.scrollY) || 0), behavior: "auto" });
    });
  }
  markBuilderSaved();
}

function restoreBuilderReturnSession() {
  ...
  try {
    ...
    console.warn("Could not restore the previous Exam Builder session.", error);
    return null;
  }
}

function captureBuilderReturnSession(exam) {
  document.querySelectorAll("[data-question-card]").forEach((card) => {
    const questionId = card.dataset.questionId;
    if (!questionId) return;
    try {
      graphDrafts.set(questionId, normalizeGraph(getGraphValuesFromCard(card)));
    } catch (_) {
      // A collapsed or not-yet-mounted editor can safely keep its last draft.
    }
  });
  try {
    ...
    console.warn("Could not store the builder return snapshot. The student view can still open.", error);
  }
}
...
function bindEvents() {
  ...
  els.addQuestionBtn.addEventListener("click", () => {
    ...
    previewQuestionIndex = state.questions.length - 1;
  ...
  els.openStudentViewBtn.addEventListener("click", () => {
    ...
    if (sizeAudit.totalBytes > ONLINE_RENDER_SIZE_LIMIT_BYTES
      && !window.confirm(buildOnlineRenderSizeWarning(sizeAudit))) {
      return;
    }
    try {
      localStorage.setItem(ACTIVE_EXAM_KEY, JSON.stringify(activeExam));
      try {
        sessionStorage.setItem("kelp-exam-viewer-role", "teacher");
      } catch (_) {
        // The active exam carries the teacher preview marker too.
      }
      captureBuilderReturnSession(activeExam);
    } catch (error) {
      console.error("Could not prepare the student view.", error);
      alert(buildOnlineRenderStorageFailureMessage(sizeAudit));
      return;
    }
    window.open("./exam-taker.html", "_blank", "noopener");
  });

  els.printExamBtn.addEventListener("click", () => {
    prepareExamForPrint().catch((error) => {
      console.error("Could not fully prepare the exam for printing.", error);
      window.print();
    });
  });
  ...
  els.refreshPreviewBtn?.addEventListener("click", () => {
    renderAllPreviews({ force: true, animatePreview: true }).catch((error) => {
      console.error("Could not refresh the live preview.", error);
      setPreviewRefreshState("stale");
      if (els.refreshPreviewBtn) els.refreshPreviewBtn.disabled = false;
    });
  });
  els.previewPreviousBtn.addEventListener("click", () => {
    previewQuestionIndex = Math.max(0, previewQuestionIndex - 1);
    renderAllPreviews({ force: true, animatePreview: true });
  });
  els.previewNextBtn.addEventListener("click", () => {
    previewQuestionIndex = Math.min(Math.max(0, state.questions.length - 1), previewQuestionIndex + 1);
    renderAllPreviews({ force: true, animatePreview: true });
  });
  ...
  bindQuestionReorderEvents();
  document.addEventListener("input", markBuilderDirty, true);
  document.addEventListener("change", markBuilderDirty, true);
  document.addEventListener("click", handleBuilderMutationClick, true);
  document.addEventListener("kelp-diagram-change", markBuilderDirty, true);
  document.addEventListener("kelp-diagram-attach", markBuilderDirty, true);
  ...
  window.addEventListener("beforeprint", prepareExamForNativePrint);
  window.addEventListener("afterprint", exitExamPrintMode);
}
...
function clearEssayAnswerPageBreaks() {
  els.examPreview?.querySelectorAll?.(".exam-paper-answer-space.needs-page-break").forEach((space) => {
    space.classList.remove("needs-page-break");
  });
}

function applyEssayAnswerPageBreaks() {
  clearEssayAnswerPageBreaks();
  if (!els.examPreview) return;

  const usableA4HeightMm = 297 - 12 - 16;
  const pageHeightPx = usableA4HeightMm * (96 / 25.4);
  const previewRect = els.examPreview.getBoundingClientRect();
  const spaces = els.examPreview.querySelectorAll(".exam-paper-answer-space.essay:not(.pdf-answer-space-custom)");

  spaces.forEach((space) => {
    const rect = space.getBoundingClientRect();
    if (!rect.height || rect.height >= pageHeightPx) return;

    const topInPreview = rect.top - previewRect.top + els.examPreview.scrollTop;
    const pageOffset = ((topInPreview % pageHeightPx) + pageHeightPx) % pageHeightPx;
    const remaining = pageHeightPx - pageOffset;
    const bufferPx = 12 * (96 / 25.4);

    if (rect.height + bufferPx > remaining) {
      space.classList.add("needs-page-break");
    }
  });
}

function enterExamPrintMode() {
  document.body.classList.add("is-exam-printing");
  if (els.examPreview) els.examPreview.scrollTop = 0;
}

function exitExamPrintMode() {
  printPreviewPrepared = false;
  clearEssayAnswerPageBreaks();
  document.body.classList.remove("is-exam-printing");
  drawAllGraphs();
  updatePreviewFollowing(document.querySelector(".exam-layout"));
}

function prepareExamForNativePrint() {
  updateMetaFromInputs();
  enterExamPrintMode();
  if (printPreviewPrepared) {
    drawAllGraphs();
    return;
  }
  previewRefreshInProgress = true;
  renderExamPreview();
  drawAllGraphs();
  previewRenderHasRun = true;
  previewNeedsRefresh = false;
  previewRefreshInProgress = false;
  setPreviewRefreshState("current");
  applyEssayAnswerPageBreaks();
  typesetMath(els.examPreview).then(() => {
    applyEssayAnswerPageBreaks();
    drawAllGraphs();
  });
}
...
async function prepareExamForPrint() {
  ...
  updateMetaFromInputs();
  enterExamPrintMode();
  await renderAllPreviews({ force: true });
...
function normalizeQuestion(question) {
  ...
  normalized.optionGraphs = normalizeOptionGraphs(question.optionGraphs, normalized.options.length);
  normalized.optionImages = normalizeOptionImages(question.optionImages, normalized.options.length);
  ...
  normalized.numericExactMatch = normalizeStoredBoolean(question.numericExactMatch);
  normalized.numericTolerance = normalizeNumericTolerance(question.numericTolerance);
  normalized.numericAngleMode = normalizeNumericAngleMode(question.numericAngleMode);
  normalized.numericRequireUnit = normalizeStoredBoolean(question.numericRequireUnit);
  normalized.numericUnit = String(question.numericUnit || "").trim();
...
function renderQuestions() {
  ...
  state.questions.forEach((question, index) => {
    ...
    node.querySelector("[data-question-heading]").textContent = getQuestionDisplayName(question);
    ...
    updatePdfAnswerSettingsOptions(node, question);
    node.querySelector('[data-field="pdfAnswerSpaceSize"]').value = normalizePdfAnswerSpaceSize(question.pdfAnswerSpaceSize, question.type);
    node.querySelector('[data-field="pdfAnswerSpaceCustomMm"]').value = normalizePdfAnswerSpaceCustomMm(question.pdfAnswerSpaceCustomMm);
    node.querySelector('[data-field="numericExpectedAnswer"]').value = question.numericExpectedAnswer || "";
    node.querySelector('[data-field="numericExactMatch"]').checked = Boolean(question.numericExactMatch);
    node.querySelector('[data-field="numericTolerance"]').value = normalizeNumericToleranceChoice(question.numericTolerance);
    node.querySelector('[data-field="numericAngleMode"]').value = normalizeNumericAngleMode(question.numericAngleMode);
    node.querySelector('[data-field="numericRequireUnit"]').checked = Boolean(question.numericRequireUnit);
    ...
    const moveUpButton = node.querySelector('[data-action="move-question-up"]');
    const moveDownButton = node.querySelector('[data-action="move-question-down"]');
    ...
    if (moveDownButton) moveDownButton.disabled = index === state.questions.length - 1;
    updateEssayPdfSettingsVisibility(node, question);
    updateNumericSettingsVisibility(node, question);
  ...
  drawAllGraphs();
  typesetMath();
  requestAnimationFrame(() => updatePreviewFollowing(document.querySelector(".exam-layout")));
}

function persistMountedQuestionGraphs() {
  document.querySelectorAll("[data-question-card]").forEach((card) => {
    const questionId = card.dataset.questionId;
    if (!questionId) return;
    try {
      ...
      // A collapsed or not-yet-mounted editor can keep its last saved draft.
    }
  });
}

function updatePdfAnswerSettingsOptions(card, question) {
  const select = card?.querySelector?.('[data-field="pdfAnswerSpaceSize"]');
  if (!select) return;
  const options = getPdfAnswerSpaceOptionsForType(question?.type);
  if (!options.length) return;
  const nextHtml = options
    .map(([value, label]) => `<option value="${escapeAttribute(value)}">${escapeHTML(label)}</option>`)
    .join("");
  if (select.dataset.optionSet === question.type && select.innerHTML === nextHtml) return;
  select.innerHTML = nextHtml;
  select.dataset.optionSet = question.type;
}

function updateEssayPdfSettingsVisibility(card, question) {
  const settings = card?.querySelector?.("[data-essay-pdf-settings]");
  const custom = card?.querySelector?.("[data-essay-pdf-custom]");
  const usesPdfBlockSettings = question?.type === "essay" || question?.type === "short-answer";
  updatePdfAnswerSettingsOptions(card, question);
  question.pdfAnswerSpaceSize = normalizePdfAnswerSpaceSize(question?.pdfAnswerSpaceSize, question?.type);
  const select = card?.querySelector?.('[data-field="pdfAnswerSpaceSize"]');
  if (select) select.value = question.pdfAnswerSpaceSize;
  const usesCustom = normalizePdfAnswerSpaceSize(question?.pdfAnswerSpaceSize, question?.type) === "custom";
  settings?.classList.toggle("is-hidden", !usesPdfBlockSettings);
  custom?.classList.toggle("is-hidden", question?.type !== "essay" || !usesCustom);
}

function updateNumericSettingsVisibility(card, question) {
  const settings = card?.querySelector?.("[data-numeric-settings]");
  const unitField = card?.querySelector?.("[data-numeric-unit-field]");
  const isNumeric = question?.type === "numeric";
  settings?.classList.toggle("is-hidden", !isNumeric);
  unitField?.classList.toggle("is-hidden", !isNumeric || !question?.numericRequireUnit);
}

function captureQuestionCardPositions() {
  return new Map([...els.questionList.querySelectorAll("[data-question-card]")].map((card) => [
    card.dataset.questionId,
    card.getBoundingClientRect()
  ]));
}

function animateQuestionReorder(previousPositions, movedQuestionId) {
  if (window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches) return;
  const cards = [...els.questionList.querySelectorAll("[data-question-card]")];
  cards.forEach((card) => {
    const previous = previousPositions.get(card.dataset.questionId);
  ...
  const movedCard = [...els.questionList.querySelectorAll("[data-question-card]")]
    .find((card) => card.dataset.questionId === movedQuestionId);
  if (movedCard) {
    movedCard.classList.add("is-question-moving");
    window.setTimeout(() => movedCard.classList.remove("is-question-moving"), 820);
  }
}

function reorderQuestion(questionId, targetQuestionId, placement = "before") {
  const fromIndex = state.questions.findIndex((question) => question.id === questionId);
  const targetIndex = state.questions.findIndex((question) => question.id === targetQuestionId);
  if (fromIndex < 0 || targetIndex < 0 || questionId === targetQuestionId) return;

  persistMountedQuestionGraphs();
  const previousPositions = captureQuestionCardPositions();
  const activePreviewQuestionId = state.questions[previewQuestionIndex]?.id;
  const [question] = state.questions.splice(fromIndex, 1);
  let insertionIndex = state.questions.findIndex((item) => item.id === targetQuestionId);
  if (placement === "after") insertionIndex += 1;
  state.questions.splice(Math.max(0, insertionIndex), 0, question);
  previewQuestionIndex = Math.max(0, state.questions.findIndex((item) => item.id === activePreviewQuestionId));

  renderQuestions();
  renderAllPreviews({ animatePreview: true });
  animateQuestionReorder(previousPositions, questionId);
}

function moveQuestionBy(questionId, delta) {
  const index = state.questions.findIndex((question) => question.id === questionId);
  const targetIndex = index + delta;
  if (index < 0 || targetIndex < 0 || targetIndex >= state.questions.length) return;
  reorderQuestion(questionId, state.questions[targetIndex].id, delta > 0 ? "after" : "before");
}

function duplicateQuestion(question) {
  const sourceIndex = state.questions.findIndex((item) => item.id === question?.id);
  ...
  if (sourceDraft) {
    graphDrafts.set(duplicate.id, normalizeGraph(cloneQuestionValue(sourceDraft)));
  }

  pendingEnterQuestionId = duplicate.id;
  previewQuestionIndex = sourceIndex + 1;
  markBuilderDirty();
  renderQuestions();
  renderAllPreviews({ animatePreview: true });
  animateToolbarFrom(toolbarBefore);
...
function clearQuestionDragState() {
  ...
  els.questionList.querySelectorAll(".is-question-dragging, .is-question-drop-before, .is-question-drop-after")
    .forEach((card) => card.classList.remove("is-question-dragging", "is-question-drop-before", "is-question-drop-after"));
}

function bindQuestionReorderEvents() {
  els.questionList.addEventListener("click", (event) => {
    const button = event.target.closest?.('[data-action="move-question-up"], [data-action="move-question-down"]');
    const card = button?.closest?.("[data-question-card]");
    if (!button || !card) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    moveQuestionBy(card.dataset.questionId, button.dataset.action === "move-question-up" ? -1 : 1);
  }, true);

  els.questionList.addEventListener("dragstart", (event) => {
    const handle = event.target.closest?.("[data-question-drag-handle]");
    const card = handle?.closest?.("[data-question-card]");
    if (!handle || !card) {
      event.preventDefault();
      return;
    }
    draggedQuestionId = card.dataset.questionId;
    card.classList.add("is-question-dragging");
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", draggedQuestionId);
  });

  els.questionList.addEventListener("dragover", (event) => {
    if (!draggedQuestionId) return;
    const targetCard = event.target.closest?.("[data-question-card]");
    if (!targetCard || targetCard.dataset.questionId === draggedQuestionId) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
    els.questionList.querySelectorAll(".is-question-drop-before, .is-question-drop-after")
      .forEach((card) => card.classList.remove("is-question-drop-before", "is-question-drop-after"));
    const placement = event.clientY < targetCard.getBoundingClientRect().top + targetCard.offsetHeight / 2 ? "before" : "after";
    targetCard.classList.add(placement === "before" ? "is-question-drop-before" : "is-question-drop-after");
  });

  els.questionList.addEventListener("dragleave", (event) => {
    const targetCard = event.target.closest?.("[data-question-card]");
    if (!targetCard) return;
    if (event.relatedTarget && targetCard.contains(event.relatedTarget)) return;
    targetCard.classList.remove("is-question-drop-before", "is-question-drop-after");
  });

  els.questionList.addEventListener("drop", (event) => {
    if (!draggedQuestionId) return;
    const targetCard = event.target.closest?.("[data-question-card]");
    if (!targetCard || targetCard.dataset.questionId === draggedQuestionId) {
      clearQuestionDragState();
      return;
    }
    event.preventDefault();
    const targetBounds = targetCard.getBoundingClientRect();
    const placement = targetCard.classList.contains("is-question-drop-after")
      ? "after"
      : targetCard.classList.contains("is-question-drop-before")
        ? "before"
        : event.clientY < targetBounds.top + (targetBounds.height / 2) ? "before" : "after";
    const sourceId = draggedQuestionId;
    clearQuestionDragState();
    reorderQuestion(sourceId, targetCard.dataset.questionId, placement);
  });

  els.questionList.addEventListener("dragend", clearQuestionDragState);
}
...
function renderOptions(card, question) {
  ...
  const usesOptions = questionUsesOptions(question);
  const allowsMultipleCorrect = questionAllowsMultipleCorrect(question);
  const optionContentType = questionOptionContentType(question);
  ...
  question.options.forEach((option, optionIndex) => {
    ...
    if (optionContentType !== "text") row.classList.add("has-option-assets");
    ...
    input.value = option;
    input.placeholder = optionContentType === "graph"
      ? `Optional label for option ${optionLetter(optionIndex)}`
      : `Option ${optionLetter(optionIndex)}`;
    input.disabled = question.type === "true-false";

    const optionActions = document.createElement("div");
    optionActions.className = "exam-option-actions";

    if (optionContentType === "graph") {
      const hasGraph = Boolean(question.optionGraphs?.[optionIndex] && graphHasContent(question.optionGraphs[optionIndex]));
      const editGraph = document.createElement("button");
      editGraph.type = "button";
      editGraph.className = hasGraph ? "btn-secondary exam-small-btn" : "btn-outline exam-small-btn";
      editGraph.dataset.action = "edit-option-graph";
      editGraph.dataset.optionIndex = String(optionIndex);
      editGraph.textContent = hasGraph ? "Edit graph" : "Create graph";
      editGraph.setAttribute("aria-label", `${hasGraph ? "Edit" : "Create"} graph for option ${optionLetter(optionIndex)}`);
      optionActions.appendChild(editGraph);

      if (hasGraph) {
        const clearGraph = document.createElement("button");
        clearGraph.type = "button";
        clearGraph.className = "btn-outline exam-small-btn";
        clearGraph.dataset.action = "remove-option-graph";
        clearGraph.dataset.optionIndex = String(optionIndex);
        clearGraph.textContent = "Clear graph";
        optionActions.appendChild(clearGraph);
      }
    } else if (optionContentType === "image") {
      const hasImage = Boolean(question.optionImages?.[optionIndex]);
      const editImage = document.createElement("label");
      editImage.className = hasImage ? "btn-secondary exam-small-btn exam-option-image-picker" : "btn-outline exam-small-btn exam-option-image-picker";
      editImage.textContent = hasImage ? "Edit image" : "Add image";
      editImage.setAttribute("aria-label", `${hasImage ? "Edit" : "Add"} image for option ${optionLetter(optionIndex)}`);

      const imageInput = document.createElement("input");
      imageInput.type = "file";
      imageInput.accept = "image/*";
      imageInput.dataset.optionImageUpload = "true";
      imageInput.dataset.optionIndex = String(optionIndex);
      editImage.appendChild(imageInput);
      optionActions.appendChild(editImage);

      if (hasImage) {
        const clearImage = document.createElement("button");
        clearImage.type = "button";
        clearImage.className = "btn-outline exam-small-btn";
        clearImage.dataset.action = "remove-option-image";
        clearImage.dataset.optionIndex = String(optionIndex);
        clearImage.textContent = "Clear image";
        optionActions.appendChild(clearImage);
      }
    }
    ...
    optionActions.appendChild(remove);

    row.append(letter, input, correct, optionActions);
...
function renderImagePreview(card, question) {
  ...
  setImageAttachMessage(
    card,
    question.imageData ? "Attached image to this question's body." : "Attach an image to this question's body."
  );
...
function setImageAttachMessage(card, text, isError = false) {
  const message = card?.querySelector?.("[data-image-attach-message]");
  if (!message) return;
  message.textContent = text || "";
  message.classList.toggle("error", Boolean(isError));
}

function findRenderedQuestionCard(questionId) {
  return [...document.querySelectorAll("[data-question-card]")]
    .find((card) => card.dataset.questionId === questionId) || null;
}

function getImageOptionTargets(sourceQuestion) {
  return [];
}

function setQuestionImageOptionMode(question) {
  const baseType = questionBaseType(question);
  if (baseType === "multiple-answer") question.type = "multiple-answer-image";
  else if (baseType === "multiple-choice") question.type = "multiple-choice-image";
}

function openImageAttachmentDestinationDialog(sourceQuestion, defaultLabel = "") {
  return new Promise((resolve) => {
    const backdrop = document.createElement("div");
    backdrop.className = "diagram-modal-backdrop exam-attach-modal-backdrop";

    const modal = document.createElement("div");
    modal.className = "diagram-modal exam-attach-modal";
    modal.setAttribute("role", "dialog");
    modal.setAttribute("aria-modal", "true");
    modal.setAttribute("aria-labelledby", "examAttachImageTitle");

    const header = document.createElement("div");
    header.className = "diagram-modal-header";
    const title = document.createElement("h3");
    title.id = "examAttachImageTitle";
    title.className = "diagram-modal-title";
    title.textContent = "Attach image";
    const description = document.createElement("p");
    description.className = "diagram-modal-description";
    description.textContent = "Attach this image to the question body.";
    header.append(title, description);

    const destinationList = document.createElement("div");
    destinationList.className = "exam-attach-destination-list";

    const bodyButton = document.createElement("button");
    bodyButton.type = "button";
    bodyButton.className = "exam-attach-destination";
    bodyButton.dataset.destinationKind = "body";
    bodyButton.innerHTML = "<strong>Exam body</strong><span>Attach to this question's image section.</span>";
    destinationList.appendChild(bodyButton);

    const bodyLabelField = document.createElement("label");
    bodyLabelField.className = "exam-attach-label-field";
    const bodyLabelText = document.createElement("span");
    bodyLabelText.textContent = "Image label / caption";
    const bodyLabelInput = document.createElement("input");
    bodyLabelInput.type = "text";
    bodyLabelInput.value = String(sourceQuestion.imageAlt || defaultLabel || "");
    bodyLabelInput.placeholder = "Example: Unit circle diagram";
    bodyLabelField.append(bodyLabelText, bodyLabelInput);
    destinationList.appendChild(bodyLabelField);

    const optionTargets = getImageOptionTargets(sourceQuestion);
    if (optionTargets.length) {
      const optionTitle = document.createElement("p");
      optionTitle.className = "exam-attach-group-title";
      optionTitle.textContent = "Answer option image slots";
      destinationList.appendChild(optionTitle);
    }

    optionTargets.forEach(({ question, index }) => {
      syncQuestionOptionAssets(question);

      const group = document.createElement("section");
      group.className = "exam-attach-question-group";

      const groupHeader = document.createElement("div");
      groupHeader.className = "exam-attach-question-header";
      const groupTitle = document.createElement("strong");
      groupTitle.textContent = `Question ${index + 1}`;
      const prompt = document.createElement("span");
      prompt.textContent = question.prompt.trim() ? firstWords(question.prompt, 9) : "Untitled question";
      groupHeader.append(groupTitle, prompt);

      const slots = document.createElement("div");
      slots.className = "exam-attach-option-slots";

      question.options.forEach((_, optionIndex) => {
        const hasImage = Boolean(question.optionImages?.[optionIndex]);
        const slot = document.createElement("button");
        slot.type = "button";
        slot.className = "exam-attach-option-slot";
        slot.classList.toggle("is-filled", hasImage);
        slot.dataset.destinationKind = "option";
        slot.dataset.questionId = question.id;
        slot.dataset.optionIndex = String(optionIndex);
        slot.innerHTML = `<strong>${optionLetter(optionIndex)}</strong><span>${hasImage ? "Filled" : "Empty"}</span>`;
        slots.appendChild(slot);
      });

      group.append(groupHeader, slots);
      destinationList.appendChild(group);
    });
    ...
    modal.append(header, destinationList, actions);
    backdrop.appendChild(modal);

    const close = (destination = null) => {
    ...
    const handleKeydown = (keyEvent) => {
      if (keyEvent.key === "Escape") close(null);
    };

    backdrop.addEventListener("click", (clickEvent) => {
      if (clickEvent.target === backdrop) close(null);
    });

    cancel.addEventListener("click", () => close(null));
    bodyButton.addEventListener("click", () => close({
      kind: "body",
      questionId: sourceQuestion.id,
      label: bodyLabelInput.value.trim()
    }));
    destinationList.addEventListener("click", (clickEvent) => {
      const slot = clickEvent.target.closest("[data-destination-kind='option']");
      if (!slot) return;
      close({
        kind: "option",
        questionId: slot.dataset.questionId,
        optionIndex: Number(slot.dataset.optionIndex)
      });
    });

    document.addEventListener("keydown", handleKeydown);
    document.body.appendChild(backdrop);
    requestAnimationFrame(() => bodyButton.focus());
  });
}
...
async function attachImageFromUpload(card, question, file, dataUrl) {
  const altText = file.name.replace(/\.[^.]+$/, "");
  if (question.imageData && !window.confirm("This question already has a body image. Replace it?")) {
    setImageAttachMessage(card, "Existing body image kept.");
    return;
  }
  question.imageData = dataUrl;
  question.imageAlt = String(question.imageAlt || altText).trim();
  const altField = card.querySelector('[data-field="imageAlt"]');
  if (altField) altField.value = question.imageAlt;
  renderImagePreview(card, question);
  renderAllPreviews();
}

async function attachOptionImageFromUpload(card, question, optionIndex, file, dataUrl) {
  if (questionOptionContentType(question) !== "image") {
    setImageAttachMessage(card, "Choose an image answer type before attaching images to answer options.", true);
    return;
  }

  syncQuestionOptionAssets(question);
  if (!Number.isInteger(optionIndex) || optionIndex < 0 || optionIndex >= question.optionImages.length) return;
  if (question.optionImages[optionIndex] && !window.confirm(
    `Option ${optionLetter(optionIndex)} already has an image. Replace it?`
  )) {
    setImageAttachMessage(card, "Existing option image kept.");
    return;
  }

  question.optionImages[optionIndex] = dataUrl;
  const questionIndex = state.questions.findIndex((item) => item.id === question.id);
  const message = `Attached image to Question ${questionIndex + 1}, option ${optionLetter(optionIndex)}.`;
  renderQuestions();
  renderAllPreviews();
  setImageAttachMessage(findRenderedQuestionCard(question.id), message);
}
...
function handleQuestionInput(event) {
  ...
  if (event.target.matches("[data-field]")) {
    if (!applyQuestionDataField(card, question, event.target)) return;
    renderAllPreviewsDebounced();
  }
...
function applyQuestionDataField(card, question, fieldElement) {
  ...
  if (field === "name" || field === "prompt") {
    ...
    if (preview) preview.textContent = question.prompt || "Type a question above to preview LaTeX.";
    card.querySelector("[data-question-heading]").textContent = getQuestionDisplayName(question);
  }
  ...
  if (field === "pdfAnswerSpaceSize" || field === "pdfAnswerSpaceCustomMm") {
    question.pdfAnswerSpaceSize = normalizePdfAnswerSpaceSize(question.pdfAnswerSpaceSize, question.type);
    question.pdfAnswerSpaceCustomMm = normalizePdfAnswerSpaceCustomMm(question.pdfAnswerSpaceCustomMm);
    updateEssayPdfSettingsVisibility(card, question);
  }

  if (field.startsWith("numeric")) {
    question.numericExpectedAnswer = String(question.numericExpectedAnswer || "");
    question.numericExactMatch = Boolean(question.numericExactMatch);
    question.numericTolerance = normalizeNumericTolerance(question.numericTolerance);
    question.numericAngleMode = normalizeNumericAngleMode(question.numericAngleMode);
    question.numericRequireUnit = Boolean(question.numericRequireUnit);
    question.numericUnit = String(question.numericUnit || "").trim();
    updateNumericSettingsVisibility(card, question);
  }
...
async function handleQuestionChange(event) {
  ...
  if (event.target.matches('[data-field="type"]')) {
    ...
    if (questionHasMeaningfulChoiceWork(question) && !window.confirm(
      "You're about to discard this question's answer choices. The question text, body image, and body diagram will stay. Continue?"
    )) {
      event.target.value = previousType;
      return;
    }
    question.type = nextType;
    resetQuestionTypeSpecificContent(question);
    if (question.type === "true-false") {
      question.options = ["True", "False"];
  ...
  if (event.target.matches("[data-image-upload]")) {
    ...
    if (uploadText) uploadText.textContent = "Preparing image...";
    try {
      const dataUrl = await readImageFileAsDataUrl(file);
      await attachImageFromUpload(card, question, file, dataUrl);
    } catch (error) {
      setImageAttachMessage(card, error.message || "Could not read this image.", true);
    } finally {
      event.target.value = "";
      if (uploadText) uploadText.textContent = "Attach image";
    }
  }

  if (event.target.matches("[data-option-image-upload]")) {
    ...
    try {
      const dataUrl = await readImageFileAsDataUrl(file);
      await attachOptionImageFromUpload(card, question, optionIndex, file, dataUrl);
    } catch (error) {
      setImageAttachMessage(card, error.message || "Could not read this image.", true);
    } finally {
      event.target.value = "";
...
function questionHasMeaningfulWork(question) {
  const draft = graphDrafts.get(question.id);
  const meaningfulOptions = question.type === "true-false"
    ? (question.options || []).some((option, index) => String(option || "").trim() !== ["True", "False"][index])
  ...
  return Boolean(
    ...
    || meaningfulOptions
    || question.imageData
    || String(question.imageBeforeText || "").trim()
    || String(question.imageAfterText || "").trim()
    || (question.graph && graphHasContent(question.graph))
    || (draft && graphHasContent(draft))
    || String(question.graphBeforeText || "").trim()
    || String(question.graphAfterText || "").trim()
    || (question.optionImages || []).some(Boolean)
    || (question.optionGraphs || []).some((graph) => graph && graphHasContent(graph))
  );
}

function questionHasMeaningfulChoiceWork(question) {
  const baseType = questionBaseType(question);
  const optionContentType = questionOptionContentType(question);
  const options = Array.isArray(question.options) ? question.options : [];
  const hasOptionText = question.type === "true-false"
    ? options.some((option, index) => String(option || "").trim() !== ["True", "False"][index])
    : options.some((option) => String(option || "").trim());
  const hasOptionGraphs = optionContentType === "graph"
    && (question.optionGraphs || []).some((graph) => graph && graphHasContent(graph));
  const hasOptionImages = optionContentType === "image"
    && (question.optionImages || []).some(Boolean);
  const hasChoiceAnswer = baseType === "multiple-answer"
    ? (question.correctOptionIndexes || []).length > 0
    : question.correctOptionIndex !== 0;
  return Boolean(hasOptionText || hasOptionGraphs || hasOptionImages || hasChoiceAnswer);
}

function resetQuestionTypeSpecificContent(question) {
  const optionCount = question.type === "true-false" ? 2 : 4;
  question.options = question.type === "true-false"
    ? ["True", "False"]
    : Array.from({ length: optionCount }, () => "");
  question.optionGraphs = Array.from({ length: optionCount }, () => null);
  question.optionImages = Array.from({ length: optionCount }, () => "");
  question.correctOptionIndex = 0;
  question.correctOptionIndexes = [];
  question.pdfAnswerSpaceSize = normalizePdfAnswerSpaceSize(question.pdfAnswerSpaceSize, question.type);
}
...
function handleQuestionClick(event) {
  ...
  if (action === "toggle-question") {
    ...
    if (!question.collapsed) collapseOtherQuestions(question, card);
    animateQuestionCardCollapse(card, question.collapsed);
    actionButton.textContent = question.collapsed ? "Maximize" : "Minimize";

    if (!question.collapsed) {
      setTimeout(drawAllGraphs, MOTION.foldMs);
    } else {
      scheduleQuestionSubmenuCollapse(question, card);
    }
...
function collapseQuestionSubmenus(question, card = null) {
  ["basic", "image", "graph"].forEach((sectionName) => {
    question[`${sectionName}Collapsed`] = true;
    if (card?.isConnected) updateFoldState(card, sectionName, true);
  });
}

function scheduleQuestionSubmenuCollapse(question, card) {
  if (!question?.collapsed) return;
  window.setTimeout(() => {
    if (!question.collapsed) return;
    collapseQuestionSubmenus(question, card);
  }, MOTION.foldMs);
}

function collapseOtherQuestions(activeQuestion, activeCard = null) {
  if (!activeQuestion) return;
  state.questions.forEach((question) => {
    if (!question || question.id === activeQuestion.id || question.collapsed) return;
    question.collapsed = true;
    const card = activeCard?.parentElement?.querySelector?.(`[data-question-id="${CSS.escape(question.id)}"]`)
      || document.querySelector(`[data-question-id="${CSS.escape(question.id)}"]`);
    const toggle = card?.querySelector?.('[data-action="toggle-question"]');
    if (toggle) toggle.textContent = "Maximize";
    if (card?.isConnected) animateQuestionCardCollapse(card, true);
    scheduleQuestionSubmenuCollapse(question, card);
  });
}

function collapseSiblingQuestionSubmenus(question, card, activeSectionName) {
  if (!question || !card || !activeSectionName) return;
  ["basic", "image", "graph"].forEach((sectionName) => {
    if (sectionName === activeSectionName) return;
    const propertyName = `${sectionName}Collapsed`;
    if (question[propertyName]) return;
    question[propertyName] = true;
    updateFoldState(card, sectionName, true);
  });
}

function animateQuestionCardCollapse(card, isCollapsed) {
  const body = card?.querySelector?.(".exam-question-body");
  if (!card || !body) {
  ...
  card.classList.add("is-question-animating");
  ...
  const cleanup = () => {
    ...
    body.style.overflow = "";
    body.style.transition = "";
    card.classList.remove("is-question-animating");
  };
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      body.style.height = opening ? `${endHeight}px` : "0px";
    });
  });

  window.setTimeout(cleanup, MOTION.foldMs + 700);
}
...
function setPreviewRefreshState(stateName) {
  const status = els.previewRefreshStatus;
  if (!status) return;
  ...
  status.dataset.previewState = stateName;
  status.classList.toggle("is-stale", stateName === "stale");
  status.classList.toggle("is-refreshing", stateName === "refreshing");
}

function markPreviewDirty() {
  if (!previewRenderHasRun || previewRefreshInProgress) return;
...
function renderAllPreviewsDebounced() {
  clearTimeout(renderTimer);
  renderTimer = setTimeout(markPreviewDirty, 180);
}

async function renderAllPreviews(options = {}) {
  if (previewRenderHasRun && !options.force) {
    markPreviewDirty();
    return;
  }
  ...
  previewRefreshInProgress = true;
  if (els.refreshPreviewBtn) els.refreshPreviewBtn.disabled = true;
  setPreviewRefreshState("refreshing");
...
function renderExamPreview() {
  ...
  previewQuestionIndex = questionCount
    ? Math.max(0, Math.min(previewQuestionIndex, questionCount - 1))
    : 0;
  const questionsHtml = state.questions
    .map((question, index) => renderQuestionPreview(question, index, index === previewQuestionIndex))
    .join("");

  if (els.previewQuestionCount) {
    els.previewQuestionCount.textContent = questionCount
      ? `Question ${previewQuestionIndex + 1} of ${questionCount}`
      : "0 questions";
  }
  if (els.previewPreviousBtn) els.previewPreviousBtn.disabled = questionCount === 0 || previewQuestionIndex === 0;
  if (els.previewNextBtn) els.previewNextBtn.disabled = questionCount === 0 || previewQuestionIndex >= questionCount - 1;
...
function renderOptionPreviewContent(question, option, optionIndex) {
  ...
  if (contentType === "graph") {
    ...
    const graphHtml = graph && graphHasContent(graph)
      ? renderGraphFigure(graph, `data-preview-option-graph-id="${question.id}" data-preview-option-index="${optionIndex}"`, `Graph option ${optionLetter(optionIndex)}`)
  ...
  if (contentType === "image") {
    const image = question.optionImages?.[optionIndex];
    return `
      <span class="exam-option-content exam-option-content-image">
        ${optionText ? `<span class="exam-option-text">${escapeHTML(optionText)}</span>` : ""}
        <span class="exam-option-media-tile">${image ? `<img src="${escapeAttribute(image)}" alt="Image option ${optionLetter(optionIndex)}" />` : '<span class="exam-option-asset-missing">No image attached.</span>'}</span>
      </span>
    `;
  }
...
function renderQuestionPreview(question, index, isActive = true) {
  const visualOptionsClass = questionOptionContentType(question) !== "text" ? " has-visual-options" : "";
  const optionsHtml = questionUsesOptions(question)
    ? `<ol class="exam-paper-options${visualOptionsClass}">${question.options.map((option, optionIndex) => `
        <li><span class="exam-option-bubble">${optionLetter(optionIndex)}</span>${renderOptionPreviewContent(question, option, optionIndex)}</li>
      `).join("")}</ol>`
  ...
  return `
    <article class="exam-paper-question${isActive ? "" : " is-preview-page-hidden"}" data-preview-question-page="${index}">
...
function renderAnswerSpace(questionOrType) {
  const type = typeof questionOrType === "string" ? questionOrType : questionOrType?.type;
  if (type === "essay" || type === "short-answer") {
    const size = normalizePdfAnswerSpaceSize(questionOrType?.pdfAnswerSpaceSize, type);
    if (size === "none") return "";
    const heightMm = getPdfAnswerSpaceHeightMm(questionOrType);
    const classes = [
      "exam-paper-answer-space",
      type === "essay" ? "essay" : "short-answer",
      `pdf-answer-space-${size}`
    ].join(" ");
    return `<div class="${classes}" style="--pdf-answer-space-height: ${escapeAttribute(String(heightMm))}mm;" data-pdf-answer-space="${escapeAttribute(size)}" aria-label="${type === "essay" ? "Essay" : "Short answer"} answer space"></div>`;
  }
...
function drawPoints(ctx, points, meta) {
  ...
  points.forEach((point, index) => {
    ...
    if (label) {
      if (!point.labelHidden) drawTextWithHalo(ctx, label, px + (point.labelDx ?? 8), py + (point.labelDy ?? -7), point.color || "#145c63");
    }
  });
...
function drawSegments(ctx, graph, meta, polygonPoints = []) {
  ...
  segments.forEach((segment) => {
    ...
    if (label) {
      ...
      if (segment.labelMode === "name") drawTextWithOverbar(ctx, label, labelX, labelY, segment.color || "#145c63");
      else drawTextWithHalo(ctx, label, labelX, labelY, segment.color || "#145c63");
    }
  });
...
function drawAngles(ctx, graph, meta) {
  ...
  graph.angles.forEach((angle) => {
    ...
    const arc = getAngleArc(from, vertex, to, meta, angle.angleMeasure);
    const radius = 22;
    ctx.beginPath();
    ctx.arc(arc.vertexPx.px, arc.vertexPx.py, radius, arc.start, arc.end, arc.counterclockwise);
    ctx.stroke();

    const label = getAngleLabel(from, vertex, to, angle.labelMode, angle.angleMeasure);
    if (label && angle.labelMode !== "blank") {
      ...
      drawTextWithHalo(ctx, label, labelX, labelY, angle.color || "#145c63");
    }
  });
...
function getAngleArc(from, vertex, to, meta, measure = "minor") {
  ...
  const arc = getSmallArc(startAngle, endAngle, measure);
  return { ...arc, vertexPx };
}
...
function computeAngleDegrees(from, vertex, to, measure = "minor") {
  ...
  const degrees = Math.acos(Math.max(-1, Math.min(1, dot / (magU * magV)))) * 180 / Math.PI;
...
function getSmallArc(start, end, measure = "minor") {
  let diff = end - start;
  while (diff <= -Math.PI) diff += Math.PI * 2;
  while (diff > Math.PI) diff -= Math.PI * 2;
  if (measure === "reflex" && diff !== 0) diff += diff > 0 ? -Math.PI * 2 : Math.PI * 2;
  return {
    start,
    end: start + diff,
    middle: start + diff / 2,
...
function drawTextWithHalo(ctx, text, x, y, color = "#145c63") {
  ctx.save();
  ctx.lineWidth = 4;
...
function drawTextWithOverbar(ctx, text, x, y, color = "#145c63") {
  ctx.save();
  ctx.lineWidth = 4;
  ctx.strokeStyle = "rgba(255, 255, 255, 0.92)";
  ctx.strokeText(text, x, y);
...
function buildExamStructure() {
  ...
  const questionCounts = {
    ...
    multipleChoiceImage: 0,
    multipleChoiceGraph: 0,
    multipleAnswerText: 0,
    multipleAnswerImage: 0,
    multipleAnswerGraph: 0,
    trueFalse: 0,
    numericAnswer: 0,
    shortAnswer: 0,
    longAnswer: 0
  ...
  exam.questions.forEach((question, questionIndex) => {
    ...
    if (baseType === "multiple-choice") {
      ...
      else if (contentType === "graph") questionCounts.multipleChoiceGraph += 1;
      else questionCounts.multipleChoiceText += 1;
    } else if (baseType === "multiple-answer") {
      if (contentType === "image") questionCounts.multipleAnswerImage += 1;
      else if (contentType === "graph") questionCounts.multipleAnswerGraph += 1;
      else questionCounts.multipleAnswerText += 1;
    } else if (question.type === "true-false") {
      questionCounts.trueFalse += 1;
    } else if (question.type === "numeric") {
      questionCounts.numericAnswer += 1;
    } else if (question.type === "short-answer") {
      questionCounts.shortAnswer += 1;
    } else if (question.type === "essay") {
      questionCounts.longAnswer += 1;
    }

    if (question.imageData) imageCount += 1;
    imageCount += (question.optionImages || []).filter(Boolean).length;

    if (question.graph && graphHasContent(question.graph)) {
      graphEntries.push({
        questionNumber: questionIndex + 1,
        questionName: getQuestionDisplayName(question),
        location: `Question ${questionIndex + 1} body diagram`,
        graph: question.graph
      });
    }

    (question.optionGraphs || []).forEach((graph, optionIndex) => {
      if (!graph || !graphHasContent(graph)) return;
      graphEntries.push({
        questionNumber: questionIndex + 1,
        questionName: getQuestionDisplayName(question),
        location: `Question ${questionIndex + 1} option ${optionLetter(optionIndex)} graph`,
        graph
      });
    });
  });

  const graphSummaries = graphEntries.map((entry) => {
    const summary = summarizeGraphObjects(entry.graph);
    return {
      questionNumber: entry.questionNumber,
      questionName: entry.questionName,
      location: entry.location,
      objectCount: summary.total,
      objectCounts: summary.counts,
      objects: summary.objects
    };
  });

  const graphWithMostObjects = graphSummaries.reduce((best, graph) => (
    !best || graph.objectCount > best.objectCount ? graph : best
  ), null);

  return {
    schema: "kelp-exam-structure-v1",
    generatedAt,
    madeBy: exam.madeBy || EXAM_MADE_BY_PLACEHOLDER,
    examName: exam.title || "",
    date: generatedAt,
    subject: exam.subject || "",
    instructions: exam.instructions || "",
    ...
    graphWithMostObjects: graphWithMostObjects || null,
    allGraphs: graphSummaries
  };
}

function summarizeGraphObjects(rawGraph) {
  const graph = normalizeGraph(rawGraph || {});
  const counts = {};
  ...
  add("points", (graph.points || []).filter((point) => point.visible !== false).length);
  if (Array.isArray(graph.functions)) {
    add("functions", graph.functions.filter((curve) => curve?.visible !== false && String(curve?.expression || "").trim()).length);
  } else if (String(graph.expression || "").trim() && graph.functionVisible !== false) {
    add("functions");
  }

  const visibleSegments = (graph.segments || []).filter((segment) => segment.visible !== false);
  add("lineSegments", visibleSegments.filter((segment) => segment.labelMode !== "length").length);
  add("distanceMeasurements", visibleSegments.filter((segment) => segment.labelMode === "length").length);
  add("angles", (graph.angles || []).filter((angle) => angle.visible !== false).length);

  if (graph.regularPolygon && graph.regularPolygon.visible !== false) add("regularPolygons");

  (graph.shapes || []).forEach((shape) => {
    if (!shape || shape.visible === false || shape.type === "latexText") return;
    if (shape.type === "irregularPolygon" && (shape.constraint === "rectangle" || /rectangle/i.test(shape.label || ""))) {
      add("rectangles");
      return;
    }
    const key = {
      regularPolygon: "regularPolygons",
      irregularPolygon: "irregularPolygons",
      circle: "circles",
      ellipse: "ellipses",
      trapezoid: "trapezoids",
      parallelogram: "parallelograms",
      circuitSymbol: "circuitSymbols"
    }[shape.type];
    add(key || "otherGraphObjects");
  });
  ...
  return {
    total: objects.reduce((sum, item) => sum + item.count, 0),
    counts,
    objects
  };
}
...
async function typesetMath(root = document.body) {
  ...
  try {
    ...
    while (!window.MathJax.typesetPromise && performance.now() - startedAt < 1500) {
...
function mergeParsedPointsWithExisting(parsedPoints, existingPoints) {
  ...
  return parsedPoints.map((point, index) => {
    ...
    const merged = { ...point };
    if (existing) {
      merged.labelDx = existing.labelDx;
      merged.labelDy = existing.labelDy;
      merged.labelHidden = existing.labelHidden;
      merged.color = existing.color;
      merged.size = existing.size;
      merged.visible = existing.visible;
      merged.coordinateLabelMode = existing.coordinateLabelMode;
      merged.coordinatePrecision = existing.coordinatePrecision;
      merged.coordinateVariable = existing.coordinateVariable;
      merged.labelBasisWidth = existing.labelBasisWidth;
      merged.labelBasisHeight = existing.labelBasisHeight;
    }
    return normalizePoint(merged, index);
  }).filter(Boolean);
}
...
function normalizeShape(shape, index = 0) {
  ...
  if (type === "circle") {
    const center = normalizeShapePoint(shape.center, { x: 0, y: 0 });
    const radius = Math.max(0.1, parseNumberOrDefault(shape.radius, 1));
    return {
      ...base,
      center,
      radius,
      radiusPoint: projectShapeCircleRadiusPoint({ center, radius, radiusPoint: shape.radiusPoint }),
...
function normalizeGraph(graph = {}) {
  ...
  return {
    ...
    functionXMin: parseOptionalNumber(graph.functionXMin),
    functionXMax: parseOptionalNumber(graph.functionXMax),
    pointsText,
    points,
    segments,
    angles,
...
function drawAllGraphs() {
  ...
  if (previewNeedsRefresh && !previewRefreshInProgress) return;
  ...
  document.querySelectorAll("[data-preview-option-graph-id]").forEach((canvas) => {
    const question = findQuestion(canvas.dataset.previewOptionGraphId);
    const optionIndex = Number(canvas.dataset.previewOptionIndex);
    const graph = question?.optionGraphs?.[optionIndex];
...
function handleQuestionClick(event) {
  ...
  if (action === "move-question-up") { moveQuestionBy(question.id, -1); return; }
  if (action === "move-question-down") { moveQuestionBy(question.id, 1); return; }
  ...
  if (action === "toggle-question") {
    ...
    else scheduleQuestionSubmenuCollapse(question, card);
    renderAllPreviewsDebounced();
    return;
  }
  ...
  if (action === "toggle-basic" || action === "toggle-image" || action === "toggle-graph") {
    ...
    const propertyName = `${sectionName}Collapsed`;
    question[propertyName] = !question[propertyName];
    if (!question[propertyName]) collapseSiblingQuestionSubmenus(question, card, sectionName);
    updateFoldState(card, sectionName, question[propertyName]);
...
function drawGraph(canvas, rawGraph) {
  ...
  const legacyPolygonPoints = graph.regularPolygon && graph.regularPolygon.visible !== false ? getRegularPolygonPoints(graph.regularPolygon) : [];
...
function drawPoints(ctx, points, meta) {
  ...
  points.forEach((point, index) => {
    ...
    ctx.fill();
    ctx.stroke();
    if (label && !point.labelHidden) drawTextWithHalo(ctx, label, px + (point.labelDx ?? 8), py + (point.labelDy ?? -7), point.color || "#145c63");
  });
...
function shapeFillStyle(shape, fallbackAlpha = 0.42) {
  const raw = Number(shape?.fillOpacity);
  const alpha = Number.isFinite(raw) ? Math.min(1, Math.max(0, raw)) : fallbackAlpha;
  return hexToRgba(shape?.fill || "#e8f7f9", alpha);
}

function drawPolygon(ctx, points, meta, polygon = {}) {
  if (!Array.isArray(points) || points.length < 3) return;
  ctx.save();
  ctx.fillStyle = shapeFillStyle(polygon, 0.42);
...
function drawSegments(ctx, graph, meta, polygonPoints = []) {
  const polygonSegments = polygonPoints.length >= 3
    ? polygonPoints.map((point, index) => {
        const next = polygonPoints[(index + 1) % polygonPoints.length];
        const key = makeSegmentKeyLabels(point.label, next.label);
        return { from: point.label, to: next.label, labelMode: graph.regularPolygon?.segmentLabelModes?.[key] || "name", polygon: true, polygonSegmentKey: key, visible: Boolean(graph.regularPolygon && graph.regularPolygon.visible !== false) };
      })
    : [];
...
function findPointByLabelInGraph(graph, label) {
  ...
  if (manual) return manual;
  const polygonPoints = graph.regularPolygon && graph.regularPolygon.visible !== false ? getRegularPolygonPoints(graph.regularPolygon) : [];
...
function drawRegularPolygonShapeSegments(ctx, shape, points, meta) {
  ...
  points.forEach((start, index) => {
    ...
    const labelX = (a.px + b.px) / 2;
    const labelY = (a.py + b.py) / 2 - 10;
    if (mode === "name") drawTextWithOverbar(ctx, label, labelX, labelY, style.color);
    else drawTextWithHalo(ctx, label, labelX, labelY, style.color);
  });
  ctx.restore();
}
...
function findClickedSegment(graph, meta, clickX, clickY) {
  ...
  if (graph.regularPolygon && graph.regularPolygon.visible !== false) {
    const polygonPoints = getRegularPolygonPoints(graph.regularPolygon);
    polygonPoints.forEach((point, index) => {
      const next = polygonPoints[(index + 1) % polygonPoints.length];
      const key = makeSegmentKeyLabels(point.label, next.label);
...
function getVisibleDiagramPoints(graph) {
  ...
  return {
    ...
    legacyPolygon: normalized.regularPolygon && normalized.regularPolygon.visible !== false ? getRegularPolygonPoints(normalized.regularPolygon) : [],
...
function drawGraph(canvas, rawGraph) {
  ...
  if (graph.expression && graph.functionVisible !== false) {
    try {
      ...
      const samples = 700;
      const hasDomainMin = String(graph.functionXMin ?? "").trim() !== "" && Number.isFinite(Number(graph.functionXMin));
      const hasDomainMax = String(graph.functionXMax ?? "").trim() !== "" && Number.isFinite(Number(graph.functionXMax));
      const domainMin = hasDomainMin ? Number(graph.functionXMin) : xMin;
      const domainMax = hasDomainMax ? Number(graph.functionXMax) : xMax;
      const startX = Math.max(xMin, domainMin);
      const endX = Math.min(xMax, domainMax);
      if (startX <= endX) {
        for (let i = 0; i <= samples; i += 1) {
...
function findNearestSnapPoint(graph, meta, cursorPx, hitPx) {
  ...
  if (graph.regularPolygon && graph.regularPolygon.visible !== false) candidates.push(...getRegularPolygonPoints(graph.regularPolygon));
  (graph.shapes || []).filter((shape) => shape.visible !== false).forEach((shape) => {
...
function findNearestSnapOnSegment(graph, meta, point, cursorPx, hitPx) {
  ...
  if (graph.regularPolygon && graph.regularPolygon.visible !== false) addPointSequence(getRegularPolygonPoints(graph.regularPolygon), true);
  (graph.shapes || []).filter((shape) => shape.visible !== false).forEach((shape) => {
...
function drawCurve(ctx, points, meta) {
  ...
  if (graph?.functionLabel && points.length) {
    ...
    if (anchor) {
      ...
      drawTextWithHalo(ctx, graph.functionLabel, p.px + 10, p.py - 10, graph.functionStroke || "#145c63");
    }
  }
  ctx.restore();
}
...
function normalizeShape(shape, index = 0) {
  ...
  if (type === "circle") {
    return {
      ...
      radiusPoint: projectShapeCircleRadiusPoint(shape),
      showCenter: Boolean(shape.showCenter),
      showRadiusPoint: Boolean(shape.showRadiusPoint)
    };
  }
...
function drawCircleShape(ctx, shape, meta) {
  ...
  if (shape.showRadiusPoint) {
    const radiusPoint = projectShapeCircleRadiusPoint(shape);
    const marker = meta.toPx(radiusPoint.x, radiusPoint.y);
    ctx.strokeStyle = "#ffffff";
    ctx.fillStyle = shape.stroke || "#145c63";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(marker.px, marker.py, 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
  }
  ctx.restore();
...
function getAngleLabel(from, vertex, to, mode, measure = "minor", variableLabel = "") {
  if (mode === "none" || mode === "blank") return "";
  if (mode === "value") return `${formatAngleValue(computeAngleDegrees(from, vertex, to, measure))}\u00b0`;
  if (mode === "variable") return String(variableLabel || "x").trim() || "x";
  return `\u2220${sanitizeGraphLabel(from.label)}${sanitizeGraphLabel(vertex.label)}${sanitizeGraphLabel(to.label)}`;
}

toCanvasMathText = function toCanvasMathTextCleanFinal(text) {
  const supers = {
    "0": "\u2070", "1": "\u00b9", "2": "\u00b2", "3": "\u00b3", "4": "\u2074",
    "5": "\u2075", "6": "\u2076", "7": "\u2077", "8": "\u2078", "9": "\u2079",
    "+": "\u207a", "-": "\u207b", "=": "\u207c", "(": "\u207d", ")": "\u207e",
  ...
  return String(text || "")
    ...
    .replace(/_\{([^{}]+)\}/g, (_, run) => convert(run, subs))
    .replace(/\^([A-Za-z0-9+\-=()])/g, (_, run) => convert(run, supers))
    .replace(/_([A-Za-z0-9+\-=()])/g, (_, run) => convert(run, subs))
    .replace(/\\([A-Za-z]+)/g, "$1");
};

function getScaledDiagramLabelOffset(source, meta, fallbackDx = 0, fallbackDy = 0) {
  const basisWidth = Math.max(0, diagramNumberOr(source?.labelBasisWidth, 0));
  const basisHeight = Math.max(0, diagramNumberOr(source?.labelBasisHeight, 0));
  const plotWidth = Math.max(0, diagramNumberOr(meta?.plotWidth, 0));
  const plotHeight = Math.max(0, diagramNumberOr(meta?.plotHeight, 0));
  return {
    dx: diagramNumberOr(source?.labelDx ?? source?.dx, fallbackDx) * (basisWidth && plotWidth ? plotWidth / basisWidth : 1),
    dy: diagramNumberOr(source?.labelDy ?? source?.dy, fallbackDy) * (basisHeight && plotHeight ? plotHeight / basisHeight : 1)
  };
}

/* ===== Final diagram compatibility: vectors and circuit symbols ===== */
function installFinalDiagramCompatibility() {
  const pointCoordinateLabelModes = new Set([
    "name",
    "variableY",
    "xVariable",
    "coordinates",
  ...
  const pointVariableLabels = ["x", "y", "w", "z", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t"];
  ...
  function isFinalLineStyle(value, options = {}) {
    return ["solid", "dashed", "dotted", ...(options.arrow ? ["arrow"] : [])].includes(value);
  }

  function pointCoordinatePrecision(value) {
    const numeric = Number(value);
    return Number.isFinite(numeric) ? Math.max(0, Math.min(3, Math.floor(numeric))) : 2;
  }

  function formatPointCoordinate(value, precision) {
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric.toFixed(pointCoordinatePrecision(precision)) : "";
  }

  function getPointDisplayLabel(point, index = 0) {
    const name = sanitizeGraphLabel(point?.label || `P${index + 1}`);
    const mode = pointCoordinateLabelModes.has(point?.coordinateLabelMode) ? point.coordinateLabelMode : "name";
    if (mode === "name") return name;
    const x = formatPointCoordinate(point?.x, point?.coordinatePrecision);
    const y = formatPointCoordinate(point?.y, point?.coordinatePrecision);
    const variable = String(point?.coordinateVariable || pointVariableLabels[index % pointVariableLabels.length]).trim();
  ...
  function normalizeCircuitTerminal(point, fallbackLabel, fallbackX, fallbackY) {
    const normalized = normalizeShapePoint(point || { x: fallbackX, y: fallbackY });
    normalized.label = sanitizeGraphLabel(normalized.label || fallbackLabel);
    normalized.x = parseNumberOrDefault(normalized.x, fallbackX);
    normalized.y = parseNumberOrDefault(normalized.y, fallbackY);
    normalized.labelHidden = point?.labelHidden !== false;
    normalized.visible = point?.visible !== false;
    normalized.size = Number.isFinite(Number(normalized.size)) ? Math.max(2, Number(normalized.size)) : 4;
    return normalized;
  }

  function normalizeCircuitConnectionPoints(shape, points, width, height, index) {
    const source = Array.isArray(shape.terminals) ? shape.terminals : [];
    if (source.length) {
      return source.map((point, terminalIndex) => normalizeCircuitTerminal(
        point,
        points[terminalIndex]?.label || `T${index + 1}${terminalIndex + 1}`,
        parseNumberOrDefault(point?.x, points[terminalIndex]?.x || 0),
        parseNumberOrDefault(point?.y, points[terminalIndex]?.y || 0)
      ));
    }
    if (shape.symbolKind !== "earth") return points.map((point) => ({ ...point, labelHidden: true }));
    const start = points[0] || { x: -width / 2, y: 0, label: `T${index + 1}1` };
    const end = points[1] || { x: width / 2, y: 0 };
    const dx = Number(end.x) - Number(start.x);
    const dy = Number(end.y) - Number(start.y);
    const length = Math.hypot(dx, dy) || 1;
    const centerX = Number(start.x) + dx / 2;
    const centerY = Number(start.y) + dy / 2;
    return [normalizeCircuitTerminal(
      null,
      start.label || `T${index + 1}1`,
      centerX + (-dy / length) * height * 0.31,
      centerY + (dx / length) * height * 0.31
    )];
  }

  const previousDefaultShapeLabelForCircuit = getDefaultShapeLabel;
  getDefaultShapeLabel = function getDefaultShapeLabelWithCircuit(type, index = 0) {
    if (type === "circuitSymbol") return "Circuit symbol";
    return previousDefaultShapeLabelForCircuit(type, index);
  };

  const previousNormalizeSegmentForVectors = normalizeSegment;
  normalizeSegment = function normalizeSegmentWithVectors(segment) {
    const normalized = previousNormalizeSegmentForVectors(segment);
    if (!normalized) return null;
    return {
      ...normalized,
      lineDash: isFinalLineStyle(segment?.lineDash, { arrow: true }) ? segment.lineDash : (normalized.lineDash || "solid"),
      showEndpointPoints: Boolean(segment?.showEndpointPoints),
      labelDx: diagramNumberOr(segment?.labelDx, 0),
      labelDy: diagramNumberOr(segment?.labelDy, -10),
      labelBasisWidth: Math.max(0, diagramNumberOr(segment?.labelBasisWidth, 0)),
      labelBasisHeight: Math.max(0, diagramNumberOr(segment?.labelBasisHeight, 0))
    };
  };

  const previousNormalizePointForCoordinateLabels = normalizePoint;
  normalizePoint = function normalizePointWithCoordinateLabels(point, fallbackIndex = 0) {
    const normalized = previousNormalizePointForCoordinateLabels(point, fallbackIndex);
    if (!normalized) return null;
    return {
      ...normalized,
      coordinateLabelMode: pointCoordinateLabelModes.has(point?.coordinateLabelMode) ? point.coordinateLabelMode : "name",
      coordinatePrecision: pointCoordinatePrecision(point?.coordinatePrecision),
      coordinateVariable: String(point?.coordinateVariable || "").trim().replace(/[^A-Za-z0-9_]/g, "").slice(0, 8),
      labelBasisWidth: Math.max(0, diagramNumberOr(point?.labelBasisWidth, 0)),
      labelBasisHeight: Math.max(0, diagramNumberOr(point?.labelBasisHeight, 0))
    };
  };

  const previousNormalizeShapeForCircuit = normalizeShape;
  normalizeShape = function normalizeShapeWithCircuit(shape, index = 0) {
    if (!shape || typeof shape !== "object") return null;
    if (shape.type !== "circuitSymbol") {
      const normalized = previousNormalizeShapeForCircuit(shape, index);
      if (normalized) {
        normalized.lineDash = isFinalLineStyle(shape.lineDash) ? shape.lineDash : (normalized.lineDash || "solid");
        if (Array.isArray(normalized.points) && Array.isArray(shape.points)) {
          normalized.points = normalized.points.map((point, pointIndex) => ({
            ...point,
            labelBasisWidth: Math.max(0, diagramNumberOr(shape.points[pointIndex]?.labelBasisWidth, 0)),
            labelBasisHeight: Math.max(0, diagramNumberOr(shape.points[pointIndex]?.labelBasisHeight, 0))
          }));
        }
        if (shape.type === "regularPolygon") {
          normalized.showCenter = Boolean(shape.showCenter);
          normalized.vertexLabelOffsets = Array.isArray(shape.vertexLabelOffsets)
            ? shape.vertexLabelOffsets.map((offset) => ({
                dx: diagramNumberOr(offset?.dx, 8),
                dy: diagramNumberOr(offset?.dy, -7),
                labelBasisWidth: Math.max(0, diagramNumberOr(offset?.labelBasisWidth, 0)),
                labelBasisHeight: Math.max(0, diagramNumberOr(offset?.labelBasisHeight, 0))
              }))
            : [];
        }
        if (shape.type === "circle") {
          ["center", "radiusPoint"].forEach((key) => {
            if (!normalized[key] || !shape[key]) return;
            normalized[key].labelBasisWidth = Math.max(0, diagramNumberOr(shape[key].labelBasisWidth, 0));
            normalized[key].labelBasisHeight = Math.max(0, diagramNumberOr(shape[key].labelBasisHeight, 0));
          });
        }
        normalized.segmentLabelOffsets = Object.fromEntries(Object.entries(shape.segmentLabelOffsets || {}).map(([key, offset]) => [key, {
          dx: diagramNumberOr(offset?.dx, 0),
          dy: diagramNumberOr(offset?.dy, -10),
          labelBasisWidth: Math.max(0, diagramNumberOr(offset?.labelBasisWidth, 0)),
          labelBasisHeight: Math.max(0, diagramNumberOr(offset?.labelBasisHeight, 0))
        }]));
      }
    ...
    const sourcePoints = Array.isArray(shape.points) ? shape.points.slice(0, 2) : [];
    const points = sourcePoints.length >= 2
      ? sourcePoints.map((point, pointIndex) => normalizeCircuitTerminal(
          point,
          `T${index + 1}${pointIndex + 1}`,
          centerX + (pointIndex === 0 ? -width / 2 : width / 2),
          centerY
        ))
      : [
          normalizeCircuitTerminal(null, `T${index + 1}1`, centerX - width / 2, centerY),
          normalizeCircuitTerminal(null, `T${index + 1}2`, centerX + width / 2, centerY)
        ];
    const terminals = normalizeCircuitConnectionPoints(shape, points, width, height, index);

    return {
      id: sanitizeObjectId(shape.id || `circuitSymbol-${index + 1}`),
      type: "circuitSymbol",
      ...
      fillOpacity: Number.isFinite(Number(shape.fillOpacity)) ? Math.min(1, Math.max(0, Number(shape.fillOpacity))) : 0.16,
      lineWidth: Number.isFinite(Number(shape.lineWidth)) ? Math.max(1, Number(shape.lineWidth)) : 2,
      lineDash: "solid",
      ...
      points,
      terminals,
      centerX: (Number(points[0]?.x) + Number(points[1]?.x)) / 2,
      centerY: (Number(points[0]?.y) + Number(points[1]?.y)) / 2
    };
  };

  const previousShapePointReferencesForCircuit = getShapePointReferences;
  getShapePointReferences = function getShapePointReferencesWithCircuit(graph) {
    const refs = previousShapePointReferencesForCircuit(graph);
    (graph.shapes || []).forEach((shape, shapeIndex) => {
      if (shape?.type !== "circuitSymbol" || shape.visible === false) return;
      [...(shape.terminals || []), ...(shape.points || [])].forEach((point, pointIndex) => {
  ...
  findPointByLabelInGraph = function findPointByLabelInGraphWithHiddenConstructionPoints(graph, label) {
    const normalized = normalizeGraph(graph);
    const cleanLabel = sanitizeGraphLabel(label).toLowerCase();
    if (!cleanLabel) return null;
    const manual = normalized.points.find((point) => sanitizeGraphLabel(point.label).toLowerCase() === cleanLabel);
    if (manual) return manual;
  ...
  const previousAllShapePointsForCircuit = getAllShapePoints;
  getAllShapePoints = function getAllShapePointsWithCircuit(shapes = []) {
    const points = previousAllShapePointsForCircuit(shapes);
    shapes.filter((shape) => shape?.type === "circuitSymbol" && shape.visible !== false).forEach((shape) => {
      points.push(...(shape.points || []));
      points.push(...(shape.terminals || []));
      const basis = getCircuitSymbolGraphBasis(shape);
      if (basis) points.push(...basis.corners);
    });
    return points.filter(Boolean);
  };

  const previousEdgePointsForCircuit = getEdgePointsForShape;
  getEdgePointsForShape = function getEdgePointsForShapeWithCircuit(shape) {
    if (shape?.type === "circuitSymbol") return shape.points || [];
    return previousEdgePointsForCircuit(shape);
  };

  function getCircuitSymbolGraphBasis(shape) {
    ...
    const px = -uy;
    const py = ux;
    const baseHalfHeight = Math.max(0.2, parseNumberOrDefault(shape.height, 1.6) / 2);
    const halfHeight = ["ammeter", "voltmeter"].includes(shape.symbolKind)
      ? Math.max(baseHalfHeight, length / 2)
      : baseHalfHeight;
  ...
  function drawVectorArrowheadFinal(ctx, from, to, color = "#145c63", width = 2) {
  ...
  function circuitPixelBasis(shape, meta) {
    ...
    const p = (along, offset) => ({
      x: center.x + ux * along + perp.x * offset,
      y: center.y + uy * along + perp.y * offset
    });
    return { start: { x: start.px, y: start.py }, end: { x: end.px, y: end.py }, center, length, ux, uy, perp, halfHeight, boxHalf, inner, p };
  }

  function drawCircuitPathFinal(ctx, basis, points) {
    if (!points.length) return;
    const first = basis.p(points[0][0], points[0][1]);
    ctx.beginPath();
    ctx.moveTo(first.x, first.y);
    points.slice(1).forEach((point) => {
      const next = basis.p(point[0], point[1]);
      ctx.lineTo(next.x, next.y);
    });
    ctx.stroke();
  }

  function drawCircuitSymbolShapeFinal(ctx, shape, meta) {
    ...
    ctx.save();
    ctx.strokeStyle = color;
    ctx.lineWidth = Math.max(1, Number(shape.lineWidth) || 2);
    ctx.lineJoin = "round";
    ctx.lineCap = "round";
    ...
    if (kind === "resistor" || kind === "variableResistor") {
      ...
      for (let index = 0; index <= steps; index += 1) {
        ...
        const offset = index === 0 || index === steps ? 0 : (index % 2 ? -halfHeight * 0.42 : halfHeight * 0.42);
        zig.push([along, offset]);
      }
      drawCircuitPathFinal(ctx, basis, zig);
      if (kind === "variableResistor") {
        ...
        ctx.stroke();
        drawVectorArrowheadFinal(ctx, from, to, color, shape.lineWidth);
      }
    } else if (kind === "battery") {
      [-1, -0.35, 0.35, 1].forEach((factor, index) => {
        const along = basis.boxHalf * factor;
        const tall = index % 2 === 0;
        const markHeight = tall ? halfHeight * 0.62 : halfHeight * 0.34;
        drawCircuitPathFinal(ctx, basis, [[along, -markHeight], [along, markHeight]]);
      });
      ...
      const coilCount = 4;
      const radius = Math.min(halfHeight * 0.54, basis.inner / (coilCount + 1));
      for (let index = 0; index < coilCount; index += 1) {
        const c = basis.p(-radius * (coilCount - 1) + index * radius * 2, 0);
        ctx.beginPath();
        ctx.arc(c.x, c.y, radius, Math.PI, 0, false);
        ctx.stroke();
      }
    } else if (kind === "earth") {
      drawCircuitPathFinal(ctx, basis, [[0, -halfHeight * 0.62], [0, halfHeight * 0.08]]);
      drawCircuitPathFinal(ctx, basis, [[-bodyInner * 0.34, halfHeight * 0.08], [bodyInner * 0.34, halfHeight * 0.08]]);
      drawCircuitPathFinal(ctx, basis, [[-bodyInner * 0.22, halfHeight * 0.28], [bodyInner * 0.22, halfHeight * 0.28]]);
      drawCircuitPathFinal(ctx, basis, [[-bodyInner * 0.11, halfHeight * 0.46], [bodyInner * 0.11, halfHeight * 0.46]]);
    } else if (kind === "capacitor") {
      drawCircuitPathFinal(ctx, basis, [[-bodyInner, -halfHeight * 0.62], [-bodyInner, halfHeight * 0.62]]);
      drawCircuitPathFinal(ctx, basis, [[bodyInner, -halfHeight * 0.62], [bodyInner, halfHeight * 0.62]]);
    }

    ctx.restore();
    if (shape.showTerminals) {
      drawPoints(ctx, (shape.terminals || shape.points || []).map((point) => ({ ...point, color, size: point.size || 4, visible: shape.visible !== false })), meta);
    }
  }

  const previousDrawSegmentsForVectors = drawSegments;
  drawSegments = function drawSegmentsWithVectors(ctx, graph, meta, polygonPoints = []) {
    const polygonSegments = polygonPoints.length >= 3
      ? polygonPoints.map((point, index) => {
          const next = polygonPoints[(index + 1) % polygonPoints.length];
          const key = makeSegmentKeyLabels(point.label, next.label);
          return {
            ...
            polygon: true,
            polygonSegmentKey: key,
            visible: Boolean(graph.regularPolygon && graph.regularPolygon.visible !== false),
    ...
    const segments = [...polygonSegments, ...((normalizeGraph(graph).segments) || [])].filter((segment) => segment.visible !== false);
    if (!segments.length) return;
    ...
    segments.forEach((segment) => {
      ...
      ctx.stroke();
      ctx.setLineDash([]);
      if (segment.lineDash === "arrow") drawVectorArrowheadFinal(ctx, { x: a.px, y: a.py }, { x: b.px, y: b.py }, color, segment.lineWidth);
      const label = getSegmentLabel(start, end, segment.labelMode, segment.precision);
      if (label) {
        const offset = getScaledDiagramLabelOffset(segment, meta, 0, -10);
        const labelX = (a.px + b.px) / 2 + offset.dx;
        const labelY = (a.py + b.py) / 2 + offset.dy;
        if (segment.labelMode === "name") drawTextWithOverbar(ctx, label, labelX, labelY, color);
        else drawTextWithHalo(ctx, label, labelX, labelY, color);
      }
      ctx.restore();
    });
    ctx.restore();
  ...
  drawPoints = function drawPointsWithCoordinateLabels(ctx, points, meta) {
    ...
    points.forEach((point, index) => {
      ...
      ctx.fill();
      ctx.stroke();
      const label = getPointDisplayLabel(point, index);
      if (label && !point.labelHidden) {
        const offset = getScaledDiagramLabelOffset(point, meta, 8, -7);
        drawTextWithHalo(ctx, label, px + offset.dx, py + offset.dy, color);
      }
    });
    ctx.restore();
  };

  const previousDrawShapesForCircuit = drawShapes;
  drawShapes = function drawShapesWithCircuitSymbols(ctx, graph, meta) {
    const normalized = normalizeGraph(graph || {});
    const circuitShapes = (normalized.shapes || []).filter((shape) => shape.type === "circuitSymbol" && shape.visible !== false);
    const otherShapes = {
      ...normalized,
      shapes: (normalized.shapes || []).filter((shape) => shape.type !== "circuitSymbol")
    };
    previousDrawShapesForCircuit(ctx, otherShapes, meta);
    (otherShapes.shapes || [])
      .filter((shape) => shape.type === "regularPolygon" && shape.visible !== false && shape.showCenter)
      .forEach((shape) => {
        drawPoints(ctx, [{
          label: "O",
          x: shape.centerX,
          ...
          size: 4,
          visible: true
        }], { ...meta, graph: normalized });
      });
    circuitShapes.forEach((shape) => drawCircuitSymbolShapeFinal(ctx, shape, meta));
  };
  ...
  try {
    ...
    console.warn("Could not refresh diagrams after installing circuit/vector support.", error);
  }
}

toCanvasMathText = function toCanvasMathTextClean(text) {
  const supers = {
    "0": "\u2070", "1": "\u00b9", "2": "\u00b2", "3": "\u00b3", "4": "\u2074",
    "5": "\u2075", "6": "\u2076", "7": "\u2077", "8": "\u2078", "9": "\u2079",
    "+": "\u207a", "-": "\u207b", "=": "\u207c", "(": "\u207d", ")": "\u207e",
...
function getShapePointReferences(graph) {
  ...
  (graph.shapes || []).forEach((shape, shapeIndex) => {
    ...
    if (shape.type === "circle") {
      if (shape.showCenter || shape.center?.label) {
        refs.push({ point: { ...shape.center, label: shape.center?.label || `${shape.label || "O"}` }, shape, shapeIndex, pointIndex: "center", source: "shape" });
      }
      if (shape.showRadiusPoint || shape.radiusPoint?.label) {
        refs.push({ point: { ...shape.radiusPoint, label: shape.radiusPoint?.label || `${shape.label || "R"}` }, shape, shapeIndex, pointIndex: "radiusPoint", source: "shape" });
      }
    }
    if (shape.type === "ellipse") {
...
function drawCircleShape(ctx, shape, meta) {
  ...
  ctx.save();
  ctx.strokeStyle = shape.stroke || "#145c63";
  ctx.fillStyle = shapeFillStyle(shape, 0.25);
  ctx.lineWidth = shape.lineWidth || 2;
  if (shape.lineDash === "dashed") ctx.setLineDash([8, 6]);
  if (shape.lineDash === "dotted") ctx.setLineDash([2, 6]);
  ctx.beginPath();
...
function normalizeShapePoint(point, fallback = { x: 0, y: 0 }) {
  ...
  if (point && Number.isFinite(Number(point.labelBasisWidth))) normalized.labelBasisWidth = Math.max(0, Number(point.labelBasisWidth));
  if (point && Number.isFinite(Number(point.labelBasisHeight))) normalized.labelBasisHeight = Math.max(0, Number(point.labelBasisHeight));
  if (point && point.labelHidden) normalized.labelHidden = true;
  return normalized;
}

function projectShapeCircleRadiusPoint(shape = {}) {
  const center = normalizeShapePoint(shape.center, { x: 0, y: 0 });
  const radius = Math.max(0.1, parseNumberOrDefault(shape.radius, 1));
...
function getRegularPolygonPoints(polygon) {
  ...
  const hidden = Array.isArray(polygon?.vertexLabelHidden) ? polygon.vertexLabelHidden : [];
...
function drawTextShape(ctx, shape, meta) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  drawTextWithHalo(ctx, shape.text, p.px, p.py, shape.fill || shape.stroke || "#145c63");
  ctx.restore();
}
...
function handleQuestionClick(event) {
  ...
  if (action === "remove-option-graph") {
    if (!Array.isArray(question.optionGraphs)) question.optionGraphs = [];
    const index = Number(actionButton.dataset.optionIndex);
    question.optionGraphs[index] = null;
    renderQuestions(); renderAllPreviews(); return;
  }
  if (action === "remove-option-image") {
    if (!Array.isArray(question.optionImages)) question.optionImages = [];
    const index = Number(actionButton.dataset.optionIndex);
  ...
  if (action === "add-option") {
    ...
    if (!Array.isArray(question.optionGraphs)) question.optionGraphs = [];
    if (!Array.isArray(question.optionImages)) question.optionImages = [];
    question.optionGraphs.push(null);
  ...
  if (action === "remove-option") {
    ...
    const index = Number(actionButton.dataset.optionIndex);
    question.options.splice(index, 1);
    if (Array.isArray(question.optionGraphs)) question.optionGraphs.splice(index, 1);
    if (Array.isArray(question.optionImages)) question.optionImages.splice(index, 1);
    if (question.options.length === 0) {
      question.options.push("");
      question.optionGraphs = [null];
      question.optionImages = [""];
...
function diagramNumberOr(value, fallback) {
  const number = Number(value);
  return Number.isFinite(number) ? number : fallback;
}
...
function normalizeSegment(segment) {
  ...
  return {
    ...
    lineWidth: Number.isFinite(Number(segment.lineWidth)) ? Math.max(1, Number(segment.lineWidth)) : 2,
    lineDash: ["solid", "dashed", "dotted"].includes(segment.lineDash) ? segment.lineDash : "solid",
    precision: diagramPrecisionDecimals(segment.precision, segment.labelMode === "length" ? 2 : 2),
...
function normalizeAngle(angle) {
  ...
  return {
    ...
    variableLabel: String(angle.variableLabel || "").trim().replace(/[^A-Za-z0-9_]/g, "").slice(0, 8),
    visible: angle.visible !== false,
    color: isHexColor(angle.color) ? angle.color : "#145c63",
    radius: Number.isFinite(Number(angle.radius)) ? Math.max(8, Number(angle.radius)) : 22,
    precision: diagramPrecisionDecimals(angle.precision, 0),
    ...
    labelBasisWidth: Math.max(0, diagramNumberOr(angle.labelBasisWidth, 0)),
    labelBasisHeight: Math.max(0, diagramNumberOr(angle.labelBasisHeight, 0))
  };
}
...
function drawSegments(ctx, graph, meta, polygonPoints = []) {
  ...
  segments.forEach((segment) => {
    ...
    const label = getSegmentLabel(start, end, segment.labelMode, segment.precision);
    if (label) {
      const labelX = (a.px + b.px) / 2 + diagramNumberOr(segment.labelDx, 0);
      const labelY = (a.py + b.py) / 2 + diagramNumberOr(segment.labelDy, -10);
      if (segment.labelMode === "name") drawTextWithOverbar(ctx, label, labelX, labelY, segment.color || "#145c63");
      else drawTextWithHalo(ctx, label, labelX, labelY, segment.color || "#145c63");
    }
    ctx.restore();
...
function drawAngles(ctx, graph, meta) {
  ...
  angles.forEach((angle) => {
    ...
    ctx.arc(arc.vertexPx.px, arc.vertexPx.py, radius, arc.start, arc.end, arc.counterclockwise);
    ctx.stroke();
    const label = getAngleLabel(from, vertex, to, angle.labelMode, angle.angleMeasure, angle.variableLabel, angle.precision);
    if (label && angle.labelMode !== "blank") {
      const offset = getScaledDiagramLabelOffset(angle, meta, 0, 0);
      const labelX = arc.vertexPx.px + Math.cos(arc.middle) * (radius + 16) + offset.dx;
      const labelY = arc.vertexPx.py + Math.sin(arc.middle) * (radius + 16) + offset.dy;
      drawTextWithHalo(ctx, label, labelX, labelY, angle.color || "#145c63");
    }
  });
  ctx.restore();
...
function getSegmentLabelAnchor(segment, graph, meta) {
  ...
  return {
    ...
    text: getSegmentLabel(start, end, segment.labelMode, segment.precision)
  };
}
...
function getAngleLabelAnchor(angle, graph, meta) {
  ...
  const offset = getScaledDiagramLabelOffset(angle, meta, 0, 0);
  return {
    x: arc.vertexPx.px + Math.cos(arc.middle) * (radius + 16) + offset.dx,
    y: arc.vertexPx.py + Math.sin(arc.middle) * (radius + 16) + offset.dy,
    text: getAngleLabel(from, vertex, to, angle.labelMode, angle.angleMeasure, angle.variableLabel, angle.precision)
  };
}
...
function drawTextShape(ctx, shape, meta) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  drawTextWithHalo(ctx, toCanvasMathText(shape.text), p.px, p.py, shape.fill || shape.stroke || "#145c63");
  ctx.restore();
}
...
function getSegmentOffsetFromContainer(container, key, meta = null) {
  const offset = container?.segmentLabelOffsets?.[key] || {};
  return getScaledDiagramLabelOffset(offset, meta, 0, -10);
}
...
function drawRegularPolygonShapeSegments(ctx, shape, points, meta) {
  ...
  points.forEach((start, index) => {
    ...
    const a = meta.toPx(start.x, start.y);
    const b = meta.toPx(end.x, end.y);
    const offset = getSegmentOffsetFromContainer(shape, key, meta);
...
function drawClosedPointShape(ctx, shape, meta) {
  ...
  if (points.length < 2) return;
  ctx.save();
  ctx.fillStyle = shapeFillStyle(shape, 0.42);
  const first = meta.toPx(points[0].x, points[0].y);
  ctx.beginPath();
  ctx.moveTo(first.px, first.py);
...
function normalizeShape(shape, index = 0) {
  ...
  const base = {
    ...
    stroke: isHexColor(shape.stroke) ? shape.stroke : "#145c63",
    fill: isHexColor(shape.fill) ? shape.fill : "#e8f7f9",
    fillOpacity: Number.isFinite(Number(shape.fillOpacity)) ? Math.min(1, Math.max(0, Number(shape.fillOpacity))) : 0.36,
    lineWidth: Number.isFinite(Number(shape.lineWidth)) ? Math.max(1, Number(shape.lineWidth)) : 2,
    lineDash: ["solid", "dashed", "dotted"].includes(shape.lineDash) ? shape.lineDash : "solid",
    constraint: shape.constraint === "rectangle" ? "rectangle" : "",
    segmentLabelModes: shape.segmentLabelModes && typeof shape.segmentLabelModes === "object" ? { ...shape.segmentLabelModes } : {},
  ...
  if (type === "regularPolygon") {
    ...
    const vertexLabelOffsets = Array.isArray(shape.vertexLabelOffsets)
      ...
      : [];
    const vertexLabelHidden = Array.isArray(shape.vertexLabelHidden)
      ? shape.vertexLabelHidden.map(Boolean).slice(0, sides)
      : [];
...
function ensurePreviewToggleMarkup(button) {
  if (!button || button.querySelector(".exam-preview-toggle-label")) return;
  button.textContent = "";
  const openLabel = document.createElement("span");
  openLabel.className = "exam-preview-toggle-label exam-preview-toggle-label-open";
  openLabel.textContent = "Minimize";
  const closedLabel = document.createElement("span");
  closedLabel.className = "exam-preview-toggle-label exam-preview-toggle-label-closed";
  closedLabel.textContent = "Preview";
  button.append(openLabel, closedLabel);
}

function setPreviewToggleState(button, isCollapsed) {
  ensurePreviewToggleMarkup(button);
  button.dataset.previewToggleState = isCollapsed ? "collapsed" : "open";
  button.setAttribute("aria-expanded", String(!isCollapsed));
...
function animateDiagramStagePositions(entries) {
  entries.forEach(({ stage, rect }) => {
    if (!stage.isConnected || typeof stage.animate !== "function") return;
...
function getPreviewBoundaryElement(layout) {
  const questionCards = [...layout.querySelectorAll("[data-question-card]")].filter((card) => {
    const rect = card.getBoundingClientRect();
    return rect.width > 0 && rect.height > 0;
  });
  return questionCards[questionCards.length - 1] || layout.querySelector(".exam-question-list") || layout.closest(".exam-builder-panel") || layout.closest(".tracks-panel") || layout;
}

function animatePreviewFollowShift(layout, sticky, fromRect) {
  if (!layout || !sticky || !fromRect || typeof sticky.animate !== "function") return;
  if (window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches) return;
  const toRect = sticky.getBoundingClientRect();
  const deltaX = fromRect.left - toRect.left;
  const deltaY = fromRect.top - toRect.top;
  if (Math.abs(deltaX) < 1 && Math.abs(deltaY) < 1) return;
  if (layout._previewFollowAnimation) layout._previewFollowAnimation.cancel();
  layout._previewFollowAnimation = sticky.animate(
    [
      { transform: `translate(${deltaX}px, ${deltaY}px)` },
      { transform: "translate(0, 0)" }
    ],
  ...
  layout._previewFollowAnimation.addEventListener("finish", () => {
    if (layout._previewFollowAnimation) layout._previewFollowAnimation = null;
  }, { once: true });
}

function updatePreviewFollowing(layout) {
  if (!layout) return;
  const sticky = layout.querySelector(".exam-preview-sticky");
  const column = layout.querySelector(".exam-preview-column");
  const previewBoundary = getPreviewBoundaryElement(layout);
  const unavailable = !sticky
    || !column
  ...
  if (unavailable) {
    layout.classList.remove("is-preview-following");
  ...
  const boundaryRect = previewBoundary.getBoundingClientRect();
  const shouldFollowCandidate = columnRect.top <= followInset;
  const previewTop = shouldFollowCandidate ? followInset : Math.max(followInset, stickyRect.top);
  ...
  if (wasFollowing !== shouldFollow) {
    animatePreviewFollowShift(layout, sticky, stickyRect);
  }
}

function schedulePreviewFollowingUpdate(layout) {
  if (!layout) return;
  if (layout._previewFollowRaf) return;
  layout._previewFollowRaf = requestAnimationFrame(() => {
    layout._previewFollowRaf = 0;
    updatePreviewFollowing(layout);
  });
}

function schedulePostPreviewToggleGraphDraw(layout) {
  if (!layout) return;
  if (layout._previewGraphDrawTimer) {
    window.clearTimeout(layout._previewGraphDrawTimer);
  }
  layout._previewGraphDrawTimer = window.setTimeout(() => {
    layout._previewGraphDrawTimer = 0;
    drawAllGraphs();
  }, 80);
}

function performPreviewColumnToggle(layout, button) {
...
function togglePreviewColumn(layout, button) {
  ...
  if (!willCollapse || !sticky || prefersReducedMotion || typeof sticky.animate !== "function") {
    performPreviewColumnToggle(layout, button);
    return;
  }

  const fixedRect = sticky.getBoundingClientRect();
  const columnRect = sticky.parentElement?.getBoundingClientRect();
...
function setupStage7Runtime() {
  ...
  if (previewButton && layout) {
    ...
    window.addEventListener("scroll", () => schedulePreviewFollowingUpdate(layout), { passive: true });
    window.addEventListener("resize", () => schedulePreviewFollowingUpdate(layout));
    schedulePreviewFollowingUpdate(layout);
  }
}
...
(function stage9Patch() {
  ...
  function stage9SegmentLabelAnchor(segment, graph, meta) {
    ...
    const b = meta.toPx(end.x, end.y);
    return {
      x: (a.px + b.px) / 2 + diagramNumberOr(segment.labelDx, 0),
      y: (a.py + b.py) / 2 + diagramNumberOr(segment.labelDy, -10),
      text: getSegmentLabel(start, end, segment.labelMode || "name", segment.precision)
    };
  }
  ...
  function stage9FindLabelHit(graph, meta, clickX, clickY) {
    ...
    for (let index = 0; index < (normalized.points || []).length; index += 1) {
      ...
      const p = meta.toPx(point.x, point.y);
      const x = p.px + diagramNumberOr(point.labelDx, 8);
      const y = p.py + diagramNumberOr(point.labelDy, -7);
      if (stage9Hit(x, y, point.label, clickX, clickY)) {
        return { kind: "manual", index, labelDx: diagramNumberOr(point.labelDx, 8), labelDy: diagramNumberOr(point.labelDy, -7) };
      }
    }
    ...
    for (let index = 0; index < (normalized.segments || []).length; index += 1) {
      ...
      if (anchor?.text && stage9Hit(anchor.x, anchor.y, anchor.text, clickX, clickY)) {
        return { kind: "segmentLabel", index, labelDx: diagramNumberOr(segment.labelDx, 0), labelDy: diagramNumberOr(segment.labelDy, -10) };
      }
    }
    ...
    for (let index = 0; index < (normalized.angles || []).length; index += 1) {
      ...
      const anchor = getAngleLabelAnchor(angle, normalized, meta);
      if (anchor?.text && stage9Hit(anchor.x, anchor.y, anchor.text, clickX, clickY)) {
        return { kind: "angleLabel", index, labelDx: diagramNumberOr(angle.labelDx, 0), labelDy: diagramNumberOr(angle.labelDy, 0) };
      }
    }
      ...
      for (let pointIndex = 0; pointIndex < points.length; pointIndex += 1) {
        ...
        if (stage9Hit(x, y, point.label, clickX, clickY)) {
          return {
            ...
            shapeIndex,
            pointIndex,
            labelDx: diagramNumberOr(point.labelDx, 8),
            labelDy: diagramNumberOr(point.labelDy, -7)
          };
        }
      }
...
(function stage10Patch() {
  ...
  drawTextShape = function drawTextShapeStage10(ctx, shape, meta) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    drawTextWithHalo(ctx, text, p.px, p.py, shape.fill || shape.stroke || "#145c63");
    ctx.restore();
  };
...
/* ===== Standalone Kelp diagram editor bridge ===== */
(function () {
  const kelpEditorsByQuestionId = new Map();
  const legacyNormalizeGraph = normalizeGraph;
  const legacyGraphHasContent = graphHasContent;
  const legacyDrawGraph = drawGraph;

  function normalizeKelpGraph(graph) {
    return window.KelpDiagramEditor?.normalizeGraph
      ? window.KelpDiagramEditor.normalizeGraph(graph)
      : legacyNormalizeGraph(graph);
  }

  normalizeGraph = function normalizeGraphWithSharedKelpRenderer(graph = {}) {
    if (Array.isArray(graph?.functions) || Array.isArray(graph?.shapes)) return normalizeKelpGraph(graph);
    return legacyNormalizeGraph(graph);
  };

  graphHasContent = function graphHasSharedKelpContent(graph) {
    if (Array.isArray(graph?.functions) || Array.isArray(graph?.shapes)) {
      const normalized = normalizeKelpGraph(graph);
      return Boolean(
        normalized.functions.some((curve) => String(curve?.expression || "").trim())
    ...
    return legacyGraphHasContent(graph);
  };

  drawGraph = function drawGraphWithSharedKelpRenderer(canvas, graph) {
    if (window.KelpDiagramEditor?.renderToCanvas) {
      try {
        if (window.KelpDiagramEditor.renderToCanvas(canvas, graph)) return;
      } catch (error) {
        console.warn("Shared Kelp diagram renderer failed; using the legacy builder renderer.", error);
      }
    }
    legacyDrawGraph(canvas, graph);
  };

  function cloneKelpGraph(graph) {
    return normalizeKelpGraph(JSON.parse(JSON.stringify(graph || {})));
  }

  function getKelpEditorCard(host) {
    return host?.closest?.("[data-question-card]") || null;
  }

  function getKelpEditorForCard(card) {
    const host = card?.querySelector?.("[data-kelp-diagram-host]");
    return host?.__kelpDiagramEditor || null;
  }

  function getKelpInitialGraph(question) {
    const rawGraph = graphDrafts.get(question.id) || question.graph || {
      graphType: "diagram",
      displayMode: "coordinate",
      title: "",
      expression: "",
    ...
    if (rawGraph.regularPolygon && !Array.isArray(rawGraph.shapes)) {
      return normalizeKelpGraph({
        ...rawGraph,
        regularPolygon: null,
        shapes: [{
          ...rawGraph.regularPolygon,
          id: "legacy-regular-polygon",
          type: "regularPolygon",
          label: rawGraph.title || "Regular polygon"
        }]
      });
    }

    return normalizeKelpGraph(rawGraph);
  }

  function setKelpDiagramMessage(card, text, isError = false) {
    const message = card?.querySelector?.("[data-kelp-diagram-message]");
    if (!message) return;
    message.textContent = text || "";
    message.classList.toggle("error", Boolean(isError));
  }

  function describeKelpAttachedGraph(graph) {
    const normalized = normalizeKelpGraph(graph);
    const pieces = [];
    if (normalized.functions.length) pieces.push(`${normalized.functions.length} function${normalized.functions.length === 1 ? "" : "s"}`);
    if (normalized.points.length) pieces.push(`${normalized.points.length} point${normalized.points.length === 1 ? "" : "s"}`);
    if (normalized.segments.length) pieces.push(`${normalized.segments.length} segment${normalized.segments.length === 1 ? "" : "s"}`);
    if (normalized.angles.length) pieces.push(`${normalized.angles.length} angle${normalized.angles.length === 1 ? "" : "s"}`);
    if (normalized.shapes?.length) pieces.push(`${normalized.shapes.length} shape${normalized.shapes.length === 1 ? "" : "s"}`);
    const name = normalized.title ? ` "${normalized.title}"` : "";
    return `Attached diagram${name}: ${pieces.join("; ") || "empty"}`;
  }

  function mountKelpDiagramEditor(card, question) {
    const host = card?.querySelector?.("[data-kelp-diagram-host]");
    if (!host || !question) return null;

    if (!window.KelpDiagramEditor) {
      host.innerHTML = '<p class="exam-error">The standalone diagram editor did not load.</p>';
      return null;
    }

    const graph = getKelpInitialGraph(question);
    ...
    const editor = new window.KelpDiagramEditor(host, { graph });
    host.__kelpDiagramEditor = editor;
    kelpEditorsByQuestionId.set(question.id, editor);
    graphDrafts.set(question.id, normalizeKelpGraph(editor.getGraph()));
    setKelpDiagramMessage(
      card,
  ...
  function disposeKelpDiagramEditors() {
    kelpEditorsByQuestionId.forEach((editor, questionId) => {
      if (!editor) return;
      try {
        graphDrafts.set(questionId, normalizeKelpGraph(editor.getGraph()));
      } catch (_) {
        // Preserve the latest stored draft if an editor is already unavailable.
      }
      editor.destroy?.();
    });
    kelpEditorsByQuestionId.clear();
  }

  const previousRenderGraphDraft = renderGraphDraft;
  renderGraphDraft = function renderGraphDraftWithKelpEditor(card, question) {
    if (typeof previousRenderGraphDraft === "function") {
      previousRenderGraphDraft(card, question);
    }
    mountKelpDiagramEditor(card, question);
  };

  const previousRenderQuestions = renderQuestions;
  renderQuestions = function renderQuestionsWithKelpEditorCleanup() {
    disposeKelpDiagramEditors();
    return previousRenderQuestions();
  };

  const previousGetGraphValuesFromCard = getGraphValuesFromCard;
  getGraphValuesFromCard = function getGraphValuesFromKelpEditor(card) {
    const editor = getKelpEditorForCard(card);
    if (editor) return normalizeKelpGraph(editor.getGraph());
    return previousGetGraphValuesFromCard(card);
  };

  const previousDrawAllGraphs = drawAllGraphs;
  drawAllGraphs = function drawAllGraphsWithKelpEditors() {
    if (typeof previousDrawAllGraphs === "function") {
      previousDrawAllGraphs();
    }
    kelpEditorsByQuestionId.forEach((editor) => {
      if (!editor?.destroyed) editor.draw?.();
    });
  };

  document.addEventListener("kelp-diagram-change", (event) => {
    const card = getKelpEditorCard(event.target);
    const question = card ? findQuestion(card.dataset.questionId) : null;
    if (!card || !question) return;
    const graph = cloneKelpGraph(event.detail?.graph || getGraphValuesFromCard(card));
    graphDrafts.set(question.id, graph);
  });
...
function getGraphOptionTargets(sourceQuestion) {
    return [];
  }

  function setQuestionGraphOptionMode(question) {
    const baseType = questionBaseType(question);
    if (baseType === "multiple-answer") question.type = "multiple-answer-graph";
    else if (baseType === "multiple-choice") question.type = "multiple-choice-graph";
  }

  function openKelpDiagramAttachDestinationDialog(sourceQuestion) {
    return new Promise((resolve) => {
      const backdrop = document.createElement("div");
      backdrop.className = "diagram-modal-backdrop exam-attach-modal-backdrop";
      ...
      modal.setAttribute("role", "dialog");
      modal.setAttribute("aria-modal", "true");
      modal.setAttribute("aria-labelledby", "examAttachDiagramTitle");

      const header = document.createElement("div");
      header.className = "diagram-modal-header";
      const title = document.createElement("h3");
      title.id = "examAttachDiagramTitle";
      title.className = "diagram-modal-title";
      title.textContent = "Attach diagram";
      const description = document.createElement("p");
      description.className = "diagram-modal-description";
      description.textContent = "Attach this diagram to the question body.";
      header.append(title, description);
      ...
      bodyButton.className = "exam-attach-destination";
      bodyButton.dataset.destinationKind = "body";
      bodyButton.innerHTML = "<strong>Exam body</strong><span>Attach to this question's diagram section.</span>";
      destinationList.appendChild(bodyButton);

      const optionTargets = getGraphOptionTargets(sourceQuestion);
      if (optionTargets.length) {
        const optionTitle = document.createElement("p");
        optionTitle.className = "exam-attach-group-title";
        optionTitle.textContent = "Answer option graph slots";
        destinationList.appendChild(optionTitle);
      }
      ...
      optionTargets.forEach(({ question, index }) => {
        ...
        question.options.forEach((_, optionIndex) => {
          ...
          slot.type = "button";
          slot.className = "exam-attach-option-slot";
          slot.classList.toggle("is-filled", hasGraph);
          slot.dataset.destinationKind = "option";
          slot.dataset.questionId = question.id;
          slot.dataset.optionIndex = String(optionIndex);
          slot.innerHTML = `<strong>${optionLetter(optionIndex)}</strong><span>${hasGraph ? "Filled" : "Empty"}</span>`;
          slots.appendChild(slot);
        });
      ...
      cancel.addEventListener("click", () => close(null));
      bodyButton.addEventListener("click", () => close({ kind: "body", questionId: sourceQuestion.id }));
      destinationList.addEventListener("click", (clickEvent) => {
        const slot = clickEvent.target.closest("[data-destination-kind='option']");
        if (!slot) return;
        close({
  ...
  function createDefaultOptionGraph(optionIndex) {
    return normalizeKelpGraph({
      graphType: "diagram",
      displayMode: "coordinate",
      title: `Option ${optionLetter(optionIndex)} graph`,
      expression: "",
      points: [],
      pointsText: "",
      segments: [],
  ...
  function openKelpOptionGraphEditor(question, optionIndex) {
    return new Promise((resolve) => {
      if (!window.KelpDiagramEditor) {
        alert("The diagram editor did not load.");
        resolve(false);
        return;
      }

      syncQuestionOptionAssets(question);
      const existing = question.optionGraphs?.[optionIndex];
      const initialGraph = existing && graphHasContent(existing)
        ? cloneKelpGraph(existing)
        : createDefaultOptionGraph(optionIndex);

      const backdrop = document.createElement("div");
      backdrop.className = "diagram-modal-backdrop exam-option-graph-modal-backdrop";

      const modal = document.createElement("div");
      modal.className = "diagram-modal exam-option-graph-modal";
      modal.setAttribute("role", "dialog");
      modal.setAttribute("aria-modal", "true");
      modal.setAttribute("aria-labelledby", "examOptionGraphTitle");

      const header = document.createElement("div");
      header.className = "diagram-modal-header";
      const title = document.createElement("h3");
      title.id = "examOptionGraphTitle";
      title.className = "diagram-modal-title";
      title.textContent = `Option ${optionLetter(optionIndex)} graph`;
      const description = document.createElement("p");
      description.className = "diagram-modal-description";
      description.textContent = "Edit the graph that appears inside this answer option.";
      header.append(title, description);

      const host = document.createElement("div");
      host.className = "kelp-diagram-host exam-option-graph-editor-host";

      const message = document.createElement("p");
      message.className = "exam-option-graph-modal-message";
      ...
      save.textContent = "Save graph";
      actions.append(cancel, save);

      modal.append(header, host, message, actions);
      backdrop.appendChild(modal);
      ...
      const setModalMessage = (text, isError = false) => {
        message.textContent = text || "";
        message.classList.toggle("error", Boolean(isError));
      };
      ...
      const saveGraph = () => {
        try {
          const graph = cloneKelpGraph(editor?.getGraph?.() || {});
          validateGraphValues(graph);
          syncQuestionOptionAssets(question);
          question.optionGraphs[optionIndex] = graph;
          const questionIndex = state.questions.findIndex((item) => item.id === question.id);
          const success = `Saved graph for Question ${questionIndex + 1}, option ${optionLetter(optionIndex)}.`;
          close(true);
          renderQuestions();
          renderAllPreviews();
          setKelpDiagramMessage(findRenderedQuestionCard(question.id), success);
        } catch (error) {
          setModalMessage(error.message || "Add at least one diagram object before saving.", true);
        }
      };

      const handleKeydown = (keyEvent) => {
        if (keyEvent.key === "Escape") close(false);
      };

      backdrop.addEventListener("click", (clickEvent) => {
        if (clickEvent.target === backdrop) close(false);
      });
      cancel.addEventListener("click", () => close(false));
      save.addEventListener("click", saveGraph);
      host.addEventListener("kelp-diagram-attach", (attachEvent) => {
      ...
      editor = new window.KelpDiagramEditor(host, { graph: initialGraph });
      host.__kelpDiagramEditor = editor;
      const attachButton = host.querySelector('[data-kelp-action="attach"]');
      if (attachButton) {
        attachButton.textContent = "Save graph";
        attachButton.title = "Save this answer option graph.";
      }
      const actionHelp = host.querySelector(".kde-stage-actions .kde-help");
      if (actionHelp) {
        actionHelp.dataset.helpText = "Diagram actions:\n- Undo / Redo: move through recent changes.\n- Delete: remove the current selection.\n- Clear: remove the whole drawing.\n- Reset view: restore both axes to -10 through 10.\n- Save graph: save this diagram inside the current answer option.";
        actionHelp.removeAttribute("title");
      }
      const shortcutHelp = host.querySelector(".kde-toolbar .kde-help");
      if (shortcutHelp) {
        shortcutHelp.dataset.helpText = "Shortcuts work only while this diagram editor is focused:\n- Shift+Q: Point\n- Shift+W: Segment\n- Shift+E: Regular polygon\n- Shift+F: Function\n- Shift+A: Angle\n- Shift+S: Select\n- Shift+R: Reset view\n- Shift+V: Move view\n- Shift+T: Stick\n- Shift+C: Clear\n- Shift+Space: Save option graph";
        shortcutHelp.removeAttribute("title");
      }
  ...
  document.addEventListener("click", (event) => {
    const button = event.target.closest?.('[data-action="edit-option-graph"]');
    if (!button) return;

    const card = button.closest("[data-question-card]");
    const question = card ? findQuestion(card.dataset.questionId) : null;
    const optionIndex = Number(button.dataset.optionIndex);
    if (!card || !question || !Number.isInteger(optionIndex)) return;

    event.preventDefault();
    openKelpOptionGraphEditor(question, optionIndex).catch((error) => {
      console.error("Could not edit the option graph.", error);
    });
  });

  document.addEventListener("kelp-diagram-attach", async (event) => {
    const card = getKelpEditorCard(event.target);
    const question = card ? findQuestion(card.dataset.questionId) : null;
    if (!card || !question) return;
    ...
    try {
      validateGraphValues(graph);
      graphDrafts.set(question.id, cloneKelpGraph(graph));

      if (question.graph && graphHasContent(question.graph)
        && !window.confirm("This question already has a body diagram. Replace it?")) {
        setKelpDiagramMessage(card, "Existing body diagram kept.");
        return;
      }
      question.graph = cloneKelpGraph(graph);
      setKelpDiagramMessage(card, describeKelpAttachedGraph(graph));
      drawAllGraphs();
      renderAllPreviews();
    } catch (error) {
      setKelpDiagramMessage(card, error.message || "Add at least one diagram object before attaching.", true);
    }
  });

  document.querySelectorAll("[data-question-card]").forEach((card) => {
    const question = findQuestion(card.dataset.questionId);
    if (question) mountKelpDiagramEditor(card, question);
  });
  drawAllGraphs();
})();

function diagramPrecisionDecimals(value, fallback = 2) {
  const normalized = typeof value === "string" ? value.trim().replace(",", ".") : value;
  const parsed = Number(normalized);
  const decimals = Number.isFinite(parsed) ? parsed : fallback;
  return Math.max(0, Math.min(3, Math.floor(decimals)));
}

function formatDiagramMeasurement(value, precision = 2) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return "";
  return Number.parseFloat(numeric.toFixed(diagramPrecisionDecimals(precision))).toString();
}

function getSegmentLabel(start, end, mode, precision = 2) {
  if (mode === "hidden") return "";
  if (mode === "length") {
    return formatDiagramMeasurement(Math.hypot(Number(end.x) - Number(start.x), Number(end.y) - Number(start.y)), precision);
  }
  if (mode === "variable") return "x";
  return `${sanitizeGraphLabel(start.label)}${sanitizeGraphLabel(end.label)}`;
}

function getAngleLabel(from, vertex, to, mode, measure = "minor", variableLabel = "", precision = 0) {
  if (mode === "none" || mode === "blank") return "";
  if (mode === "value") return `${formatDiagramMeasurement(computeAngleDegrees(from, vertex, to, measure), precision)}\u00b0`;
  if (mode === "variable") return String(variableLabel || "x").trim() || "x";
  return `\u2220${sanitizeGraphLabel(from.label)}${sanitizeGraphLabel(vertex.label)}${sanitizeGraphLabel(to.label)}`;
}
...
(function setupViewportHelpPopovers() {
  let popover = null;
  let activeButton = null;
  let pinnedButton = null;

  document.querySelectorAll(".exam-help-button[title]").forEach((button) => {
  ...
  function hide(button = null) {
    if (button && pinnedButton === button) return;
  ...
  function show(button, pin = false) {
    ...
    const fitsAbove = buttonRect.top >= tipRect.height + gap + margin;
    const fitsBelow = window.innerHeight - buttonRect.bottom >= tipRect.height + gap + margin;
    const placeAbove = wantsAbove ? fitsAbove : (!fitsBelow && fitsAbove);
    const left = Math.max(margin, Math.min(buttonRect.right - tipRect.width, window.innerWidth - tipRect.width - margin));
    const top = placeAbove
      ? buttonRect.top - tipRect.height - gap
      : Math.min(window.innerHeight - tipRect.height - margin, buttonRect.bottom + gap);
...
installFinalDiagramCompatibility();
