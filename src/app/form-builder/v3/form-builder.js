(() => {
  ...
  function bindStaticEvents() {
    ...
    els.formPreview.addEventListener('click', (event) => {
      ...
      if (action === 'previous') previewPageIndex -= 1;
      if (action === 'next') previewPageIndex += 1;
      renderPreview();
    });
  ...
  function buildPreviewPages() {
    ...
    if (!pages.length) pages.push({ type: 'empty', block: null, questions: [] });
    return pages;
  }
  ...
  function renderPreviewPage(page) {
    ...
    if (page.type === 'empty') {
      return `${header}<div class="form-builder-preview-empty"><p>Add blocks in the editor to build the respondent view.</p></div>`;
    }
    ...
    if (page.type === 'greeting') {
      return `${header}
        <div class="form-builder-preview-confirmation">
          ...
          <button type="button" class="form-builder-preview-cta" data-preview-action="next">${escapeHtml(page.block.buttonText || 'Start form')}</button>
        </div>
      `;
    }
...
function fieldError(key,value){if(key==='fullName'){const words=String(value||'').trim().split(/\s+/).filter(Boolean);return words.length>=2&&words.every(word=>word.length>=2)?'':'Enter at least a first and last name.'}if(key==='birthdate'){if(!value)return 'Enter a valid birthdate.';const date=new Date(value+'T00:00:00');const today=new Date();return Number.isNaN(date.getTime())||date>today?'Enter a birthdate in the past.':''}if(key==='email'){return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(String(value||'').trim())?'':'Enter an e-mail address in a valid format.'}if(key==='phone'){const digits=String(value||'').replace(/\D/g,'');return digits.length>=8&&digits.length<=15?'':'Enter a phone number with 8 to 15 digits.'}return ''}
function validatePage(){const all=pages();const page=all[pageIndex];const errors=[];if(pageIndex===firstQuestionIndex()){const config=formState.meta.respondentFields||{};respondentFields().forEach(key=>{const rules=config[key];const value=answers.fields[key]||'';if(rules.required&&!String(value).trim())errors.push('Please complete '+definitions[key].label+'.');else if(String(value).trim()){const message=fieldError(key,value);if(message)errors.push(message)}})}if(page.type==='questions'||page.type==='phase'){page.questions.filter(q=>q.required).forEach(q=>{const value=answers.questions[q.id];if(Array.isArray(value)?value.length===0:!String(value||'').trim())errors.push('Please answer '+(q.prompt||'this question')+'.')})}if(!errors.length)return true;const error=document.getElementById('error');if(error)error.textContent=errors[0];return false}
