(() => {
      ...
      <article class="form-builder-block-card${block.collapsed ? ' is-collapsed' : ''}" data-block-id="${escapeAttribute(block.id)}">
        <header class="form-builder-block-header">
          <button
            ...
            data-drag-handle
            draggable="${['greeting', 'goodbye'].includes(block.kind) ? 'false' : 'true'}"
            aria-label="${['greeting', 'goodbye'].includes(block.kind) ? `${kindLabels[block.kind]} stays in its fixed position` : 'Drag block to reorder'}"
            title="${['greeting', 'goodbye'].includes(block.kind) ? `${kindLabels[block.kind]} stays in its fixed position` : 'Drag to reorder'}"
            ${['greeting', 'goodbye'].includes(block.kind) ? 'disabled' : ''}
          >⠿</button>
  ...
  function handleBlockDrop(event) {
    ...
    if (!source || !target || ['greeting', 'goodbye'].includes(source.kind) || ['greeting', 'goodbye'].includes(target.kind)) return;

    const [movedBlock] = state.blocks.splice(sourceIndex, 1);
    let insertionIndex = targetIndex + (dropAfter ? 1 : 0);
    if (sourceIndex < insertionIndex) insertionIndex -= 1;
    state.blocks.splice(insertionIndex, 0, movedBlock);
