(() => {
  ...
  function normalizePhaseAppearance(input) {
    ...
    return {
      ...
      stripeColor: normalizeHexColor(source.stripeColor, defaults.stripeColor),
      selectionColor: normalizeHexColor(source.selectionColor, defaults.selectionColor)
    };
  }
  ...
  function phaseAppearanceVariables(input) {
    ...
    return [
      `--phase-background:${appearance.backgroundColor}`,
      `--phase-background-soft:${hexToRgba(appearance.backgroundColor, 0.11)}`,
      `--phase-stripe:${appearance.stripeColor}`,
      `--phase-selection:${appearance.selectionColor}`,
      `--phase-selection-soft:${hexToRgba(appearance.selectionColor, 0.12)}`,
      `--phase-selection-ring:${hexToRgba(appearance.selectionColor, 0.26)}`
    ].join(';');
  }
  ...
  function renderBlockCard(block, index, questionNumber) {
    ...
    return `
      <article class="form-builder-block-card${block.kind === 'phase' ? ' form-builder-phase-card' : ''}${block.collapsed ? ' is-collapsed' : ''}" data-block-id="${escapeAttribute(block.id)}" ${block.kind === 'phase' ? phaseAppearanceStyleAttribute(block.appearance) : ''}>
    ...
    return `
      <section class="form-builder-route-summary${isConditional ? '' : ' is-normal'} form-builder-phase-summary" ${phaseAppearanceStyleAttribute(appearance)}>
        ...
        <div class="form-builder-phase-colour-key" aria-label="Phase appearance">
          <span class="form-builder-phase-colour-swatch form-builder-phase-colour-swatch-background" title="Background colour"></span>
          <span class="form-builder-phase-colour-swatch form-builder-phase-colour-swatch-stripe" title="Stripe colour"></span>
          <span class="form-builder-phase-colour-swatch form-builder-phase-colour-swatch-selection" title="Selection colour"></span>
          <small>Custom appearance</small>
        </div>
        <div class="form-builder-inline-actions">
          <button type="button" class="form-builder-btn form-builder-btn-secondary form-builder-small-btn" data-action="configure-phase">Configure phase, routing and appearance</button>
        </div>
      </section>
  ...
  function renderPreviewPage(page) {
    ...
    if (page.type === 'phase') {
      return `${formHeader}<section class="form-builder-preview-phase" ${phaseAppearanceStyleAttribute(page.block.appearance)}>${content}</section>`;
    }

    return `${formHeader}${content}`;
  }
      ...
      <section class="form-builder-phase-appearance-controls">
        <div class="form-builder-phase-appearance-heading">
          <div>
            ...
            <p>Use soft tints for the page background, a stripe for structure, and a separate colour for selection and focus.</p>
          </div>
          <span class="form-builder-phase-appearance-badge">Optional design</span>
        </div>
        <div class="form-builder-phase-colour-grid">
          ...
          <label class="form-builder-phase-colour-control">
            ...
            <input type="color" data-appearance-field="stripeColor" value="${escapeAttribute(appearance.stripeColor)}" aria-label="Phase accent stripe color" />
          </label>
          <label class="form-builder-phase-colour-control">
            <span>Selection / hover</span>
            <input type="color" data-appearance-field="selectionColor" value="${escapeAttribute(appearance.selectionColor)}" aria-label="Phase selection and hover color" />
          </label>
        </div>
        <div class="form-builder-phase-appearance-sample" data-phase-appearance-sample ${phaseAppearanceStyleAttribute(appearance)}>
          <span class="form-builder-phase-sample-stripe"></span>
          <div>
            <strong>Phase preview</strong>
            <small>Subtle tint, clear stripe, and a selection cue.</small>
          </div>
          <button type="button" disabled>Selected answer</button>
        </div>
      </section>
    `;
  }
...
function hex(v,f){return /^#[0-9a-f]{6}$/i.test(String(v||''))?String(v).toUpperCase():f}function rgba(hexValue,alpha){const clean=hex(hexValue,'#00ACC1').slice(1);const r=parseInt(clean.slice(0,2),16),g=parseInt(clean.slice(2,4),16),b=parseInt(clean.slice(4,6),16);return 'rgba('+r+','+g+','+b+','+alpha+')'}function phaseVars(appearance){appearance=appearance||{};const background=hex(appearance.backgroundColor,'#00ACC1'),stripe=hex(appearance.stripeColor,'#5FAE63'),selection=hex(appearance.selectionColor,'#00ACC1');return '--phase-background:'+background+';--phase-background-soft:'+rgba(background,.11)+';--phase-stripe:'+stripe+';--phase-selection:'+selection+';--phase-selection-soft:'+rgba(selection,.12)+';--phase-selection-ring:'+rgba(selection,.26)}
